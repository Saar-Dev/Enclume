...
// ref_equipment_skill_assoc (« compétence d'utilisation ») n'a jamais été peuplée pour la
// quasi-totalité du catalogue d'armes — table distincte de ref_equipment_skills (« compétences
// boostées/requises »), jamais alimentée par aucun seed/migration depuis sa création (migration
// 48). Les 25 lignes trouvées en base provenaient de tests manuels ponctuels via l'admin API
// (routes/equipment.js), jamais reliées à la donnée source. Voir docs/JOURNAL6.md.
//
// Source : docs/ExtractCOMP.md (colonne « Compétence associée », 139 armes) — chaque nom
// recoupé avec ref_equipment.name (139/139 matchés, 0 orphelin, 0 doublon), chaque libellé
// recoupé avec ref_skills.id (11 libellés, tous vérifiés existants). WEAPON_SKILL_PAIRS généré
// par script depuis ce fichier, jamais retapé à la main.
//
// 3 corrections confirmées par Saar sur des items hors périmètre du fichier (déjà en base,
// valeurs jugées incorrectes après vérification croisée avec REGLECOMPETENCE.md) :
//  - TMP II : Fusil/Armes d'épaules (erroné) → Armes lourdes + Tir automatique.
...
//    — REGLECOMPETENCE.md p.191 cite littéralement le lance-flamme comme exemple de la
//    compétence distance, jamais contact ; confirmé par ref_skills.description pour
//    ARMES_SPECIALES_DISTANCE_COORDINATION_PERCEPTION. category='Lanceur'/fire_mode='RL'.
...
async function getItemIds(knex, names) {
  ...
  for (const n of names) {
    if (!map[n]) throw new Error('Item introuvable : ' + n)
  }
  return map
}
