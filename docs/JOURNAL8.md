...
**Origine** : Saar (liste de bugs du 2026-09-24) — « l'AOE du Klauss à ajuster en cône ». RAW `REGLES_ARMES_SPECIALES.md` : « la zone d'effet de la gerbe, de forme conique ». Avant : l'aperçu montrait un couloir rectangulaire en escalier (1/2/3/3 m) et la résolution testait la largeur du palier de chaque cible — deux formes qui ne coïncidaient pas, et aucune n'était un cône.

**Décision de règle (Saar)** : un cône **lisse** qui **ne contredit pas** le tableau ; l'angle est ajusté pour coller au tableau.

**Première calibration, rejetée par Saar à l'aperçu** : cône issu du canon, largeur du tableau lue à la limite **lointaine** de chaque palier (pente 1/7, ≈ 8,17°, jamais plus large que le tableau). Résultat : ±0,15 m à 2 m, une aiguille — « un fusil à pompe à courte portée, c'est l'antithèse de tout ce que je connais ». Mon critère « jamais plus large que le tableau » sacrifiait la courte portée, que l'analyse à charge avait pourtant signalée comme risque. Abandonnée.

**Calibration retenue** : largeur du tableau lue à la limite **proche** de chaque palier. Le cône est la droite passant par (limite proche du premier palier, sa largeur) et (limite proche du premier palier au plafond, largeur plafond). Klauss (`ref_range` 2/7/14/28 (35), largeurs 1/2/3/3) : (2 m, 1 m) et (14 m, 3 m) → pente 1/6, **≈ 9,53°**, apex virtuel **4 m derrière le tireur** (tronc de cône), plafonné à 3 m dès 14 m. Largeurs : 1,0 m à 2 m ; 1,33 m à 4 m ; 1,83 m à 7 m ; 2,33 m à 10 m ; 3 m dès 14 m. Jamais plus étroit que le tableau (sauf ≤ 0,17 m entre 7 et 8 m), au plus **1 m plus large** en fin de palier (0,83 m à 7 m, 1,0 m à 14 m). Écart au RAW assumé et voulu : aucune forme lisse ne peut égaler un escalier, et Saar préfère une zone qui déborde à une zone qui manque.

**Décisions d'architecture (analyse à charge)** :
- **Renversement assumé de `PLAN_AOE.md` v9 §2 et de sa passe 3** (« cône rejeté, deux passes par largeur de palier », 2026-09-03) : l'objection était « sous- ou sur-estimation selon le point de calibration ». Le calibrage à la limite proche ne sous-estime plus (hors ≤ 0,17 m).
- **Autorité unique** : `shared/combatRange.js#resolveShotgunCone(ref_range)` dérive angle, recul d'apex, limite proche et plafond du tableau `SHOTGUN_SPREAD_BY_BAND` et des seuils de l'arme — rien n'est recopié dans `aoe_profile` (aucune migration). Le serveur (`shotgunSpread.js` : forme = tronc de cône, touche = tronc ∩ couloir de 3 m) et l'aperçu client (`aoePreviewShape.js#projectShotgunConeTriangles`, trapèze + couloir, dispatché par mécanisme dans `Canvas3D.jsx`) la lisent tous deux. Le **palier** (dé de dégâts, Chance) reste décidé par la distance (`resolveShotgunSpread`), jamais par la forme.
- **Apex virtuel porté par la forme `cone`** (`apexBackM`, `shared/world/aoeShapes.js`) et non par un déplacement d'origine : la distance et le palier de chaque cible sont mesurés depuis `aoeShape.origin` (`queryTokensInShape`) et doivent rester ceux depuis le tireur. Le test exclut tout point derrière le tireur et mesure la portée depuis lui.
- Une `ref_range` sans deux points exploitables (ex. portée unique) lève une erreur explicite (« profil de zone d'effet invalide »), sans repli vers l'ancien couloir. Code mort retiré : `buildShotgunSpreadSegments`, `projectShotgunSpreadCorners`.
- Seul écart aperçu ↔ test : le bout à 35 m est plat à l'écran, en arc côté serveur (≤ 3 cm).

**Trouvailles notées, hors périmètre (à ticketer)** : (1) une cible à moins de 2 m (bout portant) n'est **jamais** touchée par un tir en zone, alors que le RAW dit « le tir ne touche qu'une cible, +1D10 » (`shotgunSpread.js`, `spread.widthM === null`) ; (2) les seuils du catalogue pour le Klauss (7/14/28/35) diffèrent des bornes RAW (7/15/25/50) — [INCONNU] si voulu ; la forme en dépend (avec les bornes RAW : apex 4,5 m, ≈ 8,8°).

**Testé** : `node --test` `shared/combatRange.test.mjs` 15/15 (dont `resolveShotgunCone` : valeurs du Klauss, encadrement par rapport au tableau, cas dégénérés), `shared/world/aoeShapes.test.mjs` 13/13 (dont `apexBackM`), `client/src/lib/aoePreviewShape.test.mjs` 20/20 (dont parité aperçu ↔ test de touche aux bords), `server/src/socket/socketCombatAoe.test.mjs` 25/25 ; `node --test 'shared/**/*.test.mjs'` 692/692 ; tests `aoeMechanisms`, `buildDeclarePayload`, aperçu, résolution AOE : 156/156 ; `node --check` ; eslint : 17 problèmes, identiques à HEAD, aucun dans la zone modifiée ; `vite build` ; `git diff --check`. **Validé par Saar en jeu réel (2026-09-24)** : aperçu et cibles touchées coïncident, le Klauss est efficace de près. **Non testé** : tireur drone / exo armé du Klauss (même chemin serveur, non éprouvé séparément). **Données** : aucune (pas de migration). **Retour arrière** : `git revert` du commit.

## Session (Dev) — 2026-09-24 — Fenêtre de statuts : les icônes n'attendent plus 2 s (Vite n'écoutait que sur IPv6)

**Origine** : Saar — « les icônes de la fenêtre de statuts mettent quelques secondes à s'afficher à l'ouverture, sauf Mort » (compte MJ). Pas un défaut de l'application : un piège de l'environnement de dev sous Windows.

**Diagnostic (instrumenté, puis retiré)** : [OBSERVÉ] toutes les écritures de statut diffusent la même mise à jour ; le panneau rend tel quel `token.statuses` (aucun traitement propre à Mort). Mort n'est « instantané » que parce que son icône est déjà affichée (badge du token mort) donc en cache ; les 15 autres n'avaient jamais été chargées. Le serveur livre un SVG en 4 ms (`curl`) ; ni long task, ni service worker (page rechargée en dur), ni serveur ne sont en cause. Une trace temporaire (événement socket `trace:status`, fichier `statusTrace.js`, entièrement retirée) a montré que **chaque requête neuve part à ~2 034 ms** de l'ouverture (`requestStart`) puis est servie en 1-6 ms, et que la sonde `fetch` d'un SVG neuf met 2,05 s même sans cache. [VÉRIFIÉ] Vite n'écoutait que sur `[::1]:5173` ; une connexion vers `127.0.0.1:5173` (port fermé) met 2,04 s à échouer sur cette machine Windows — la même durée. Vite ferme les connexions inactives à 5 s (`Keep-Alive: timeout=5`) : après une pause, chaque nouvelle connexion payait ce délai. [HYPOTHÈSE confirmée par l'effet] Le navigateur tente `127.0.0.1` avant `::1` ; les mesures de connexion du navigateur (`connectMs` 0) ne le montrent pas, c'est l'effet du correctif qui tranche.

**Correctif** : `client/vite.config.js`, `server.host: '127.0.0.1'` (loopback IPv4 seulement, aucune exposition au réseau local). Mesuré : sonde 2 050 ms → 124 ms, premières requêtes à 17 ms au lieu de 2 034 ms, les 16 icônes affichées en ~125 ms. Vite se recharge seul à la modification de sa config. Effet **général** au développement local : toute requête neuve après une pause (API, images, modèles 3D) gagne ces 2 s ; aucun effet en production.

**À retenir** : sous Windows avec Node ≥ 17, `localhost` peut viser `::1` alors que le navigateur essaie `127.0.0.1` : un serveur de dev qui n'écoute que sur l'une des deux familles fait payer ~2 s à chaque nouvelle connexion. Le serveur Express (`3001`) écoute déjà sur `0.0.0.0` et `::`. `curl http://[::1]:5173` échoue désormais (attendu) ; les scripts `start.ps1` / `start.sh` sondent `http://localhost:5173`, qui répond.

**Testé** : mesures `curl` (4 ms en `127.0.0.1`, échec 2,03 s en `[::1]` après le changement, 3 ms en `[::1]` avant) ; trace navigateur avant / après (2 050 ms → 124 ms) ; `node --check` ; suppression complète de la trace (`grep` sans résultat). **Validé par Saar** : « instantanée ». **Non testé** : Linux / serveur distant (le réglage ne concerne que le serveur de dev Vite ; la prod sert le build). **Données** : aucune. **Retour arrière** : retirer `host: '127.0.0.1'` de `client/vite.config.js`.

## Session (Dev) — 2026-09-25 — Fenêtres d'Annonce : un seul code de masquage pendant la sélection d'une cible ou d'un déplacement

**Origine** : Saar — « toutes les fenêtres d'action (Humain / Drone / Exo, MJ compris) doivent avoir le même code pour se masquer le temps de la sélection d'une cible ou d'un déplacement (au clic sur Déplacement ou Attaque) ». Constat [OBSERVÉ] : la règle était codée trois fois avec des conditions différentes (Humain : drapeau local `inMoveMode` + état partagé ; Exo : état partagé seul ; MJ : drapeau local `isSelectingOnMap` + ciblage par clic), et la tuile « Déplacement » ne masquait rien (elle ne faisait que réarmer le survol).

**Décision d'architecture** : autorité unique `client/src/lib/useDeclareWindowHiding.js` (cœur pur `isDeclareWindowHidden`, testé), appelée par `CombatActionWindow`, `CombatExoActionWindow` et `CombatGmDeclareWindow` (+ leur satellite d'état). Masquée si l'état partagé de la carte concerne un de ses tokens : ciblage, visée de zone, destination posée, ou sélection de déplacement **explicite** (clic sur la tuile Déplacement, Retraite, Charge). Le pilote couvre aussi le drone télépiloté. Le marqueur explicite est attaché à l'objet `combatMoveMode` armé (patron « ajustement d'état pendant le rendu », pas d'effet) : validation ou annulation le vident, le survol ambiant qui se réarme ne masque rien.

**Inchangé, décisions antérieures maintenues** : le simple survol ambiant ne masque pas (« option 1 », COMBAT-DEPLACEMENT-HOVER) ; le clic direct sur un token adverse ne masque pas (2026-07-31). Le drapeau local `inMoveMode` de la fenêtre Humain est supprimé. Seul reste côté MJ `holdHidden: isSelectingOnMap` pour l'enchaînement de cibles CaC multiples (`combatTargetMode` retombe à `null` un instant entre deux cibles).

**Hors périmètre** : le panneau MJ « Phase Annonce » (`CombatPnjPanel`), non concerné.

**Testé** : `node --test client/src/lib/useDeclareWindowHiding.test.mjs` 11/11 ; `npm run build` client ; eslint : aucune nouvelle erreur (les `set-state-in-effect` restants sont préexistants, lignes non modifiées) ; `git diff --check`. **Validé par Saar en jeu (2026-09-25)** : « tests all ok ». **Non testé** : aucun scénario connu restant. **Données** : aucune. **Retour arrière** : `git revert` du commit.

## Session (Dev) — 2026-09-25 — Recherche tolérante : un module unique pour toutes les recherches d'Enclume

**Origine** : Saar — « recherche (en particulier inventaire) plus tolérante, en particulier aux symboles spéciaux (é, è, ô, ï…) ». Constat [OBSERVÉ] : sept champs de recherche comparaient `toLowerCase().includes(q)` (taper « epee » ne trouvait pas « Épée »), et la normalisation d'accents était recopiée à la main dans quatre autres fichiers.

**Décision** : autorité unique `shared/textSearch.js` (client + serveur). `foldForSearch` : casse, accents, **ligatures** (œ → oe, æ → ae, ß → ss), **apostrophes** typographiques (’ ‘ ʼ ´ ` → '), tirets (`\p{Pd}`) et espaces multiples. `createSearchMatcher` / `matchesSearch` : tous les mots de la requête présents dans les champs, dans n'importe quel ordre (« pompe fusil » trouve « Fusil à pompe »). `foldAccents` : primitive minimale pour la classification technique. Ligatures et apostrophes ajoutés sur demande de Saar. **Pas de recherche floue** (fautes de frappe), hors périmètre.

**Branchés sur le module** : catalogue d'ajout de l'inventaire (nom + catégorie + famille), catalogue d'équipement, suggestions d'échange, sélecteur de cartes et dossiers, joueurs de `DocumentModal`, palette d'objets de l'éditeur de monde, suggestions `/t` du chat (`SidebarChatTab`) et résolution serveur de `/t` (`skillTestService.js`, égalité stricte sur la forme normalisée — devient tolérante aux ligatures, apostrophes et tirets en plus).

**Volontairement à part** : `materialDecision.js` et le classement de blueprints de `SurfaceEditorPanel` (portes / ascenseurs / échelles) utilisent `foldAccents` et non `foldForSearch` : ce sont des classifications de libellés techniques, pas des recherches ; replier les tirets y changerait des poids de textures (`sol-station` ↔ `sol station`). Comportement inchangé, seule la copie de la normalisation disparaît. Hors périmètre aussi : le filtre `ILIKE` serveur de `ticketService` (`cluster_label`, admin) et le tri de `WeaponPanel`.

**Testé** : `node --test shared/textSearch.test.mjs` 14/14 ; `materialDecision.test.mjs` ; `node --test 'shared/**/*.test.mjs'` 736/736 ; `node --check` de `skillTestService.js` ; eslint ciblé sur les 8 fichiers client touchés : aucune erreur ; `npm run build` client ; `git diff --check`. **Validé par Saar en jeu (2026-09-25)** : « test ok » (saisie réelle dans les champs). **Non testé** : `resolveSkillTestCommand` de bout en bout (aucun test existant, base requise) — la résolution serveur de `/t` n'a été éprouvée que par les suggestions client. **Données** : aucune. **Retour arrière** : `git revert` du commit.

## Session (Dev) — 2026-09-25 — Suggestions d'objets manquants à l'ajout d'inventaire (lot 1 : munitions, sacs, ceintures)

**Origine** : Saar — « suggestion de recherche » : si une arme (à distance, ou de contact qui consomme des charges) est possédée sans munitions, proposer les munitions adaptées en premier ; si un sac manque, proposer des sacs ; puis, à terme, un kit de base. Recherche de bonnes pratiques (NN/g, Algolia Recommend, UX d'inventaire de jeux) : suggestions dès l'ouverture avant toute frappe, 5-8 maximum, **justification affichée**, règles déterministes (pas de statistiques d'achat disponibles ici), récupération après « aucun résultat » (lot ultérieur).

**Décisions (Saar)** : justification affichée sur chaque suggestion (« Aucune munition 5.56 mm pour AX 56. »). **Pas de badge « Suggéré »** dans les résultats de recherche (jugé trop chargé) : les suggestions vivent dans un bloc distinct avant toute frappe, la recherche reste une recherche pure. Munition standard d'abord, puis prix croissant. La règle « sac plein » est vraie tant que `capacity` est en kg comme le poids des objets (vérifié : `ContainerPanel` affiche `{capacity} kg`).

**Données [VÉRIFIÉ en base]** : le lien arme ↔ munition est `ref_equipment.caliber` (le serveur refuse une munition d'un autre calibre) ; **8 armes de contact** ont un calibre (`Charge électrique` ×7, `GP-B2` ×1) et se traitent par la même règle ; 13 armes à distance et les armes de contact « 1 heure de gaz » n'ont pas de calibre → aucune suggestion ; le calibre « Spécial » (Mousquet déchireur) n'a aucune munition au catalogue → aucune suggestion. **`ref_equipment_ammo_compat` est vide** (0 ligne) et n'est lue nulle part : trouvaille notée, **à ticketer** (le formulaire admin l'alimente pour rien) ; ne pas s'en servir.

**Architecture** : `shared/itemSuggestions.js` (cœur pur testé, aucun texte FR : clés i18n + paramètres) ; règle unique `ammoMatchesWeapon` dans `shared/ammoRules.js`, qui remplace les quatre comparaisons de calibre recopiées (`inventoryService.js` ×2, `WeaponPanel.jsx`, `CombatActionWindow.jsx`). **Changement de comportement assumé** : deux calibres absents ne sont plus « compatibles » (`null !== null` était faux avant, `null === null` vrai côté serveur — cas absurde d'une arme sans calibre). `GET /equipment` expose désormais `caliber` et `capacity` (additif ; l'affichage du calibre à l'ajout, déjà codé, ne s'affichait jamais). « Chargeur vide » = `weaponAmmoStatus` (même autorité que fiche et combat). Détail : `docs/SYSTEME/CHARACTER.md`.

**À reprendre** : la charge d'un sac est calculée avec `computeTotalWeight`, formule identique à `containerFillKg` / `containerState` que le chantier « Prise en main » est en train d'ajouter à `shared/inventoryMath.js` (non commité au moment de ce lot) — quand il l'est, brancher `itemSuggestions.js` dessus pour n'avoir qu'une formule.

**Chantier arrêté ici (Saar, 2026-09-25)** : on teste le lot 1 en jeu et on verra s'il faut l'améliorer ; le lot 2 n'est pas lancé.
