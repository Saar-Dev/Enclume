...
### Dette DEPLACEMENT3 — Premier appel "Déplacement" après un déplacement validé : ~0,5-1s perceptible

**Symptôme** : Saar signale (2026-07-29, non gênant) un petit temps de rafraîchissement (0,5-1s) au premier clic sur "Déplacement" suivant un déplacement déjà validé. Confirme la piste notée dans DEPLACEMENT1 (clos) : le cache du compile structurel (`getBattlemapStructuralSnapshotWithRuntimeState`, `worldService.js`) est correctement invalidé par `runtime_revision`, mais `runtime_revision` est aussi incrémenté par un simple déplacement de token (`worldMovementService.js:288`, comportement documenté et voulu — `MOTEUR_MONDE.md` §2.2) alors que la position d'un token n'affecte jamais la structure compilée. Le premier appel qui suit chaque déplacement validé recompile donc une fois, même si rien de structurel n'a changé.

**Code impliqué** : `server/src/services/worldMovementService.js` (bump de `runtime_revision` sur simple déplacement), `worldService.js` (cache consommateur).

**Cause racine [HYPOTHÈSE, non instrumentée]** : conflation d'un seul compteur (`runtime_revision`) pour deux usages distincts — concurrence/staleness (légitime, large, 17 fichiers consommateurs) et invalidation du cache structurel (ne devrait dépendre que des changements de portes/ascenseurs/effets, pas de la position des tokens).

**Prochaine étape** : Non prioritaire ("rien de gênant" — Saar). Si un jour traité : séparer un compteur structurel dédié (`world_feature_states`/`world_effect_instances` uniquement), sans toucher au rôle de `runtime_revision` pour la concurrence. Chantier séparé, pas un correctif ponctuel.

---

### Dette TOURTRANSITION1 — Latence perceptible à l'enregistrement d'un déplacement en chaînant des actions de PNJ

**Symptôme** : Saar signale (2026-07-29, non gênant) un moment de latence à l'enregistrement du premier déplacement, avec affichage "En attente de {{nom}}…" (`combat.json:160`, `actionWindow.awaitingPlayer`) — alors qu'il enchaîne lui-même plusieurs actions de PNJ (pas un vrai temps d'attente d'un autre joueur humain).

**Code impliqué** : `client/src/components/CombatActionWindow.jsx:772-782` — ce message s'affiche dès que `phase === 'ANNOUNCEMENT'` et que ce n'est pas le tour du token courant d'annoncer (`currentDeclarer` = token du slot d'annonce actif, `computedAnnounceTokenId`). Générique par conception (ne distingue pas PNJ/joueur), déclenché à chaque transition de slot d'annonce.

**Cause racine [INCONNU]** : pas encore investigué — pourrait être une vraie latence réseau/serveur à la transition de phase, ou simplement le temps normal d'affichage du message avant que le slot suivant (le PNJ que Saar contrôle) ne devienne actif. Aucune reproduction ni instrumentation faite.

**Prochaine étape** : Non prioritaire ("rien de gênant" — Saar). Si un jour traité : reproduire en chaînant plusieurs PNJ, instrumenter la transition de slot d'annonce côté serveur (socket) pour distinguer latence réseau réelle vs affichage normal.
