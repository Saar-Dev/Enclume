...
## Base commune escaliers + règles Saar Session 156 (2026-07-18)

La troisième intégration commune réunit sans réécrire les espaces de travail :

- moteur monde `1255b37` (`dev/monde`, Session 158), avec escalier droit, colimaçon, palier haut,
  trémies multipolygones, rotation avant pose et tokens ancrés par leur socle ;
- règles `1733aaa` (`origin/dev/Saar`, Session 156), avec effets de statut configurables,
  propriété des personnages, échange MJ, compétences/attributs, DSL munitions, inventaire à
  emplacements multiples, visée localisée et fondations des boucliers ;
- intégration précédente `ee3302c` comme point de départ, sauvegardée avant mutation.

Les responsabilités ne changent pas : `surface_data` et `shared/world` compilent la géométrie ;
les services `worldMovementService`, `worldSpatialQueryService` et `worldVisibilityService`
restent les seules autorités spatiales consommées par le combat. Les cinq migrations Saar
`160/162/164/166/168` complètent les migrations monde sans collision de numéro ni lecture directe
