...
├── shared/
│   ├── polarisUtils.js                 # Modifié 61 — +calcAN, calcAllureMoy, calcAllures (exports partagés PI11). Modifié 65 Sprint GM-B : +DEFAULT_PNJ_ALLURES { lente:4, moyenne:8, rapide:16, max:24 }. Modifié 129 — +evaluateSalaryFormula(formula) pour calcul salaire wizard. NB : calcSkillCost + getMaxMasteryByYears présents mais CODE MORT (1er consommateur = rework Step4 Lot 1, voir PLAN_REWORKFINAL).
