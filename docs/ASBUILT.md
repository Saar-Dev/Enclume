...
| 111_ref_career_skills_fk | Ajoute `FOREIGN KEY (skill_id) REFERENCES ref_skills(id) ON DELETE RESTRICT` sur `ref_career_skills` (PIÈGE 1 corrigé pour cette table) + `DROP COLUMN skill_group` (texte libre jamais aligné avec `ref_skills.family`, source d'un bug de fragmentation UI). Backend `getStep4RefData` : JOIN `ref_skills` pour `family`. Frontend `CareersAllocator.jsx` : regroupement par `sk.family`. Détail : `docs/PLAN_CAREER_SKILLS_FK.md` (Session 134 suite) |
| 112-116_seed_ref_careers_lot{3,4a,4b,5,6} | Seed lots 3-6 : 27 carrières (marchand → voleur_criminel), illustration incluse directement dans chaque migration. Bug `required_genotype` corrigé (`techno_hybride` : valeur inventée → `TEC_HYB`). Total 37/37 carrières, 0 orphelin FK. Détail : `docs/PLAN_LOTS_3_6_CAREERS.md` (Session 134 suite) |
