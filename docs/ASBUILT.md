...
> Dernière mise à jour : 2026-07-11 Session 141 (suite 16)
...
│   │   │   ├── advantageService.js     # NOUVEAU 129 — getAdvantages (JOIN+soft-delete filter) + addAdvantage (trx-or-db pattern, valide+insère+ledger) + removeAdvantage (soft-delete+décrement). Utilisé par creation.js (step5 batch) et char-sheet.js (campagne). Modifié 141 (suite 6) — addAdvantage résout campaignId (jointure char_sheet→characters) + settings.polaris_latent, passés à validateAdvantage (6ᵉ arg). Modifié 141 (suite 9) — +getAdvantageNotes/addAdvantageNote/removeAdvantageNote (table dédiée char_advantage_notes, AdvantagesPanel.jsx Lot C, aucun coût PC, pas de soft-delete).
