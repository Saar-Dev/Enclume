...
│   │   │   ├── characters.js           # Modifié 59 — type dans GET/POST/PUT/broadcastCharacterUpdate. Modifié 139 — filtre brouillons wizard de GET / : gate sur `char_sheet.wizard_locked_at IS NULL` (au lieu de `creation_state != 'complete'`) — invariant documenté en commentaire (voir reconcileCreation). Modifié 147 — POST/PUT : calcul local type/color remplacé par `resolveOwnership()` (characterOwnershipService.js) ; PUT gagne au passage une vérification d'appartenance à la campagne sur réassignation (absente jusqu'ici). Voir `docs/Old/PLAN_CHARACTER_SERVICE.md`.
│   │   │   ├── textures.js
│   │   │   ├── assets.js
│   │   │   ├── users.js
│   │   │   ├── dice.js
...
│   │   │   ├── advantageService.js     # NOUVEAU 129 — getAdvantages (JOIN+soft-delete filter) + addAdvantage (trx-or-db pattern, valide+insère+ledger) + removeAdvantage (soft-delete+décrement). Utilisé par creation.js (step5 batch) et char-sheet.js (campagne). Modifié 141 (suite 6) — addAdvantage résout campaignId (jointure char_sheet→characters) + settings.polaris_latent, passés à validateAdvantage (6ᵉ arg). Modifié 141 (suite 9) — +getAdvantageNotes/addAdvantageNote/removeAdvantageNote (table dédiée char_advantage_notes, AdvantagesPanel.jsx Lot C, aucun coût PC, pas de soft-delete). Modifié 141 (suite 29) — +grantAdvantage(sheetId, advantageId, acquiredDuring) NOUVEAU : octroi narratif MJ post-création, mêmes contraintes qu'addAdvantage moins le budget (validateAdvantage+skipBudgetCheck), aucun contact char_pc_ledger, retour aplati identique à getAdvantages(). removeAdvantage corrigé : ne décrémente le ledger que si acquired_during==='creation_step5' (bug latent fermé avant de devenir actif). Modifié 143 — addAdvantage/grantAdvantage : `if (advantageId === 'adv_076') {...}` codé en dur ×2 remplacé par `applyIdentityGrant(trx, sheetId, refAdv.mod_identity)` (identityService.js, générique, corrige au passage adv_002 Ambidextre jamais appliqué) ; removeAdvantage : `mod_identity` ajouté au select, `recomputeIdentity(trx, sheetId, Object.keys(modIdentity))` remplace le hardcode `is_fertile:false`. Voir `docs/Old/PLAN_MUTATION2.md` Lot 6 (archivé, chantier clos).
│   │   │   ├── mutationService.js      # NOUVEAU 141 (suite 9) — getMutations/addMutation/removeMutation (char_mutations, source='campaign'). AdvantagesPanel.jsx Lot D — MJ octroie une mutation en jeu à un personnage déjà verrouillé, aucun coût PC. addMutation mirrors l'upsert stackable de creationService.js STEP3 (ON CONFLICT raw SQL, index partiel) + override char_archetype.sex/is_fertile. removeMutation = soft-delete status='removed'. Modifié 143 — addMutation : override sex/is_fertile délégué à `applyMutationIdentityGrant` (identityService.js). removeMutation passé en transaction, joint ref_mutations (mod_sex/mod_fertility de la mutation retirée), appelle `recomputeIdentity` si elle en déclarait — ferme l'asymétrie (l'override n'était jamais annulé au retrait). Voir `docs/Old/PLAN_MUTATION2.md` Lot 6 (archivé, chantier clos).
│   │   │   ├── identityService.js      # NOUVEAU 143 — résolveur sex/is_fertile/hand_pref, source commune mutations (char_mutations.mod_sex/mod_fertility) et avantages (ref_advantages.mod_identity JSONB). applyIdentityGrant/applyMutationIdentityGrant : écriture directe à l'ajout (comme aujourd'hui, généralisé). recomputeIdentity(trx, sheetId, fields) : recalcule uniquement les champs passés en paramètre depuis les sources actives restantes (mutations puis avantages, l'avantage l'emporte en cas de conflit), défaut fixe sinon (hand_pref 'R', is_fertile false, sex 'homme', décisions Saar) — jamais un recompute en bloc (écraserait un sex/hand_pref Step1 non concerné). Utilisé par mutationService.js, advantageService.js, creationService.js (STEP3+STEP5). Voir `docs/Old/PLAN_MUTATION2.md` Lot 6 (archivé, chantier clos).
│   │   │   ├── creationService.js      # NOUVEAU 129 — wizard steps 4+5. Snapshot-before rollback. Background skills additifs. Career skills SET via skillAllocations (ref_career_skills sans bonus). Validations carrière : prérequis/génotype/attributs/éducation. Salary depuis ref_career_titles. restoreSnapshot : upsert + purge orphans (whereNotIn). Modifié 136 — +getStep3RefData (ref_mutations + ref_mutation_subtypes/subtable + ref_mutation_skills imbriqués par mutation_id, pattern Map identique à getStep4RefData) ; startCreation +randomMutationsEnabled (settings.random_mutations). Modifié 139 — `finalizeCreation` → `reconcileCreation` (pattern reconciliation Kubernetes/Terraform, chaque bloc STEP1-5 conditionné à sa présence dans le payload, rejouable : reset `is_fertile`/`char_skills`/`char_careers`/`char_advantages`+ledger avant réapplication, effets d'âge en `update` absolu au lieu d'`increment`) ; +`lockWizard(sheetId)` (pose `char_sheet.wizard_locked_at`) ; +`getCharacterPreview(characterId, isGm)` (lecture brouillon, colonnes identiques à characters.js sans `worst_wound_severity`). Voir docs/STE6_FINAL.md. Modifié 141 — startCreation +randomProAdvantagesEnabled (settings.random_pro_advantages) : gate le bloc UI "Tirage 1D10" (CareersAllocator.jsx) via creationStore.js/Step4Experience.jsx, même pattern que randomMutationsEnabled — aucune revalidation serveur ajoutée dans reconcileCreation (identique au précédent). Modifié 141 (suite 2) — startCreation +skillMaxLevelEnabled (settings.skill_max_level) ; bloc STEP4 récupère désormais campaignId/settings (pattern identique STEP1) et transmet skillMaxLevelEnabled au ctx de computeSkillAllocation (shared/careerSkills.js) — revalidation serveur réelle, pas client-only (contrairement à randomProAdvantagesEnabled) car le plafond était déjà appliqué en dur côté serveur avant ce fix. Modifié 141 (suite 4) — startCreation +youngPenaltyEnabled (settings.young_penalty) ; bloc STEP4 requête char_attributes (FOR/PRE base_level) juste avant getAgeEffects(finalAge, ctx) (shared/polarisUtils.js) — réutilise le `settings` déjà chargé pour skill_max_level. Modifié 143 — STEP3/STEP5 (wipe-and-reinsert) gagnent une garde "champs identité concernés par les mutations/avantages actifs avant le `.del()`" + `recomputeIdentity` final (identityService.js) — ferme un bug latent où un retravail Wizard (désélection d'une mutation à sexe/fécondité ou d'un avantage à hand_pref) laissait `sex`/`is_fertile`/`hand_pref` bloqués sur une ancienne valeur. Voir `docs/Old/PLAN_MUTATION2.md` Lot 6 (archivé, chantier clos). Modifié 147 — startCreation : `type: 'pj'` codé en dur remplacé par `resolveOwnership(trx, {campaignId, userId})` (characterOwnershipService.js) + `color` désormais dérivée (absente jusqu'ici). Voir `docs/Old/PLAN_CHARACTER_SERVICE.md`.
│   │   │   └── characterOwnershipService.js # NOUVEAU 147 — resolveOwnership(db, {campaignId, userId}) : autorité unique type ('pj'/'pnj')/color dérivée de `campaign_members.role` (liste blanche `role==='player'` → pj, tout le reste → pnj — `role` n'a pas de contrainte CHECK en base). Corrige un bug où un personnage assigné au GM lui-même (user_id non-null mais propriétaire non-joueur) était classé PJ à tort — 3 call sites unifiés (`characters.js` POST/PUT, `creationService.js` startCreation), auparavant 3 réimplémentations indépendantes de la même règle. `DEFAULT_CHARACTER_COLOR` exporté (déduplique la couleur PNJ par défaut). Voir `docs/Old/PLAN_CHARACTER_SERVICE.md`.
│   │   └── lib/
│   │       ├── AppError.js
│   │       ├── minio.js
│   │       ├── diceParser.js
...
| 108_fix_ref_mutations_encoding | Corrige la corruption d'encodage (mojibake CP1252/UTF-8) sur `ref_mutations`/`ref_mutation_subtypes`/`ref_mutation_skills` insérée par le seed 95. Transformation déterministe et réversible (`decodeMojibake`/`encodeMojibake`, plage 0x80-0x9F). Coexiste avec `108_seed_ref_careers_lot2` (numéro dupliqué, tables disjointes, voir P53 dans `docs/SYSTEME/CORE.md`) (Session 135) |
| 109_mutation_stacking | `ref_mutations.stack_deltas` (JSONB nullable) sur les 9 lignes à incrément non-linéaire (Peau renforcée, Purulence, Squelette renforcé, Résistance naturelle ×6). Réécriture `char_mutation_effects_view` (`SUM(base + (count-1) × COALESCE(stack_deltas->>col, base))`). Coexiste avec `109_seed_ref_careers_illustration_lot2` (numéro dupliqué, voir P53 dans `docs/SYSTEME/CORE.md`) (Session 135) |
...
### Client
> ⚠️ 2026-08-30 : le corps des 3 fenêtres de déclaration a été refait (RW_DECLARE_DESIGN, `docs/Old/`).
> Voir `docs/SYSTEME/REACT.md` P58 pour l'état courant : `CombatDeclareActionList` (D5), reducers purs
> `useAssaultDeclaration`/`useMeleeDeclaration`, satellite, tokens `--decl-*`. Les lignes ci-dessous
> décrivent la structure d'avant (mode de combat, cibles multiples, reset `has_announced`) — encore
> exactes sur la logique, plus sur le rendu (tuiles → liste d'armes).
