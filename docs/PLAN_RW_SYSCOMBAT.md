...
| **Lot 1** | Noyau `computeAttackRoll` (§2.1.b-d) + assemblage contributions dans les deux fonctions + shadow-mode (§2.3) + tests unitaires (§2.2) | Faible — comportement identique bit-à-bit, aucune écriture DB ni émission déplacée | **Phase shadow codée (2026-07-25)** — 9 tests unitaires OK, fuzz d'équivalence inline/noyau 500+500 tirages sans écart, syntaxe serveur OK ; ⚠️ en attente : session de jeu Saar (CaC + Tir, modificateurs cumulés) sans `[DBG-DECOUPLAGE]`, puis commit de clôture retirant bloc inline + dispositif |
