...
| P65 | La règle de promotion d'une ligne de blessures est `isWoundLinePromoted` : la blessure qui remplirait la dernière case convertit, SAUF la ligne Mortelle (dépassement seulement — sinon toute Mortelle à 1 case deviendrait Mort). Une propriété RAW indexée par gravité se lit dans les tables partagées (`BLESSURE_SEUILS_TABLE`, `BLESSURE_EFFETS_TABLE`), jamais dans un drapeau parallèle (`is_lethal` supprimé) | BLESSURES.md |
