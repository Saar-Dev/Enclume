...
| Porte (ouvrir, fermer, crocheter) | ✅ **livré et validé en session réelle (2026-09-02)** — `CONNECTOR_ACTION_REQUEST`/`_RESOLVE` (`socketConnector.js`), Test Systèmes de sécurité arbitré MJ via `gmArbitratedTestService` (partagé avec l'interaction d'entité), `lockDifficultyDc` autoré par porte (repli −5), override MJ instantané, état runtime `world_feature_states` | `.claude/rules/world.md` § « Interaction runtime sur une porte » ; historique `JOURNAL8.md` 2026-09-02 (plan archivé `docs/Old/PLAN_INTERACTIONS_CONNECTEURS.md`) |
| Option des serrures (reconnaissance optique/digital/ADN/vocal, fusible) | 🔲 **gap ouvert** — V1 porte ne connaît que `closed`/`open`/`locked` + un Test Systèmes de sécurité ; RAW « toutes nécessitent un ordinateur pour être neutralisées » non mécanisée. À rattacher au chantier Informatique (`PLAN_INFORMATIQUE.md`) | RAW transcrite 2026-09-10 : `REGLES/REGLE_SERRURE.md` |
| Échelle (traversée verticale sans clic dédié) | 🔲 **gap ouvert** — explicitement hors périmètre du chantier porte ; `[INCONNU]` si `shared/world/navigation.js` couvre déjà la traversée verticale. À cadrer séparément | — |
