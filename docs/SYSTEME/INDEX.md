...
| `PLANS/PLAN_CLIC_3D_UNIFICATION.md` | Fusion de la détection de clic 3D — tokens (boucle manuelle `Canvas3D.jsx`) et entités (`onClick` R3F `EntityMesh.jsx`) sont deux systèmes indépendants sans arbitrage commun | Stub 2026-09-17, correctif intérimaire posé (token participe à l'arbitrage R3F), cadrage détaillé non commencé |
