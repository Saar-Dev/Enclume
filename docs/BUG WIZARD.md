...
5	Étape 7 – L’âge progresse à chaque test sans jamais régresser	Analysé	Cause : Le serveur stocke l’âge final dans char_archetype.age mais getStep4State le restitue comme âge de base ; le client le réutilise comme base, provoquant un cumul à chaque cycle. Correctif : stocker l’âge de base séparément (nouvelle colonne base_age) ou renvoyer un âge de base fixe (16 ans) dans getStep4State.
...
7	Playground – Augmentation possible des compétences limitatives sans prérequis	Analysé	Cause : L’option skill_prerequisites est désactivée par défaut ; sans elle, ni le client ni le serveur ne vérifient SKILL_MIN. Correctif : Activer l’option par défaut, ou rendre la vérification indépendante de l’option.
...
12	Mutation Parasite : jet de dé non implémenté	Analysé	Cause : rollOneMutation ne contient pas de logique pour le sous-jet 1d4. Correctif : ajouter un tirage 1d4 dans rollOneMutation et stocker le résultat dans count.
13	Avantages & revers : aucun broadcast vers le MJ	Analysé	Cause : useEffect de diffusion live dans Step4Experience.jsx ne liste pas proAdvantages ni randomPicks comme dépendances. Correctif : ajouter ces champs aux dépendances.
...
16	Traduction manquante : « Sens diminué (hearing) », « Faiblesse naturelle (drug) » et autres.	Analysé	Cause : Données en anglais dans ref_advantages.name. Correctif : traduire les termes entre parenthèses dans les seeds de la table ref_advantages.
...
19	Main directrice modifiable manuellement après un tirage RAW	Analysé	Cause : Le <select> reste actif après tirage ; l’option Ambidextre est accessible sans coût. Correctif : désactiver le <select> après tirage ou le remplacer par un affichage texte ; retirer l’option ‘A’.
20	Bouton « Suivant » grisé sans explication si le nom est vide	Analysé	Cause : Aucun message n’indique que le nom est requis. Correctif : ajouter un message conditionnel « Entrez un nom pour continuer ».
21	Carte « Aucune mutation » trop peu visible (Step 3)	Analysé	Correctif : améliorer le contraste et ajouter une icône.
22	Bouton Suivant placé tout en bas en mode Autodidacte (Step 4)	Analysé	Cause : La barre de navigation est dans le flux normal et repoussée par AutodidacteAllocator. Correctif : rendre la navigation sticky ou limiter la hauteur de l’allocateur.
23	Grille de répartition des compétences visible avant sélection d’une profession	Analysé	Correctif : conditionner l’affichage du board à selectedCareers.length > 0.
