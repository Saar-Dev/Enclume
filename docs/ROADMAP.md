...
| Informatique (ordinateurs + pannes électroniques + IEM) | `PLANS/PLAN_INFORMATIQUE.md` + `MANUELS/MANUEL_INFORMATIQUE.md` | **Lot 1 (catalogue) + Lot 2 (déclencheur IEM, cibles PJ/PNJ) FAITS, committés (`64c6667`, `dev/Saar`, pas poussé), VALIDÉS EN COMBAT RÉEL par Saar 2026-09-16.** Repository pattern `runPanneTest` (`char_inventory`/`exo_computers`) en place. **Lot 2bis (ciblage IEM sur exo-armure — Exosquelette/Générateur/Systèmes auxiliaires/Armement) CODÉ 2026-09-16, pas encore validé en jeu réel** : corrige une erreur du PLAN (les colonnes d'Intégrité existent déjà sur `exo_systems`/`exo_weapons`/`exo_sheet`, juste jamais branchées à un Test de panne) — 4 adaptateurs Repository, migrations 350-352 (`malfunction_severity`), tirage équipondéré 1/4 entre les 4 catégories (décision Saar), 33/33 tests `integrityService.test.mjs` verts. Gap satellite trouvé et non traité (hors périmètre) : un exo/drone tireur ne peut charger aucune munition typée (IEM ou autre), infrastructure absente côté Exo-armures. Lot 3a (ressource) fait avant cette session. **Lot 3b (machine à états immobilisation/redémarrage/séquelle) re-simplifié 2026-09-16, testable maintenant que le 2bis fournit un déclencheur réel** : le malus de séquelle vit sur une colonne persistante `exo_computers.sequelle_malus` (cumulatif, RAW ne prévoit pas d'effacement) plutôt que de changer la signature d'une fonction partagée par 6+ sites — pas encore codé. Lot 4 (auto-désactivation Gestion systèmes) cadré, indépendant, pas codé | Ordre restant : validation jeu réel du Lot 2bis (Saar) → Lot 3b (Survie I.E.M.) → Lot 4 (indépendant, peut se faire avant ou après). Détail architecture à jour dans `PLAN_INFORMATIQUE.md` §4 |
