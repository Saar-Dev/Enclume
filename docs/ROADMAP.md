...
| **Résolution de zone d'effet (AOE) — priorité #1 (ordre du 2026-08-26)** | `PLANS/PLAN_AOE.md` (v8, §12 tient l'avancement réel à jour — cette ligne était en retard, corrigée 2026-09-02) | **Rafraîchi 2026-09-02 (analyse depuis PLAN_AOE.md §12, pas déduit)** : étapes 1-8 codées et testées pour le **fusil à pompe, tireur PNJ** (géométrie, requête spatiale, LOS, `combat_action_targets`, payload `COMBAT_ACTION_DECLARE`, orchestration couche 4 — jamais de scénario réel exécuté contre PostgreSQL cependant). **Tireur PJ bloqué** : attend un rework de séparation des fenêtres de modificateurs/confirmation par type de combattant (DRONE/HUMAN/EXO-ARMURE), pas encore planifié (le chantier "RW déclaration" clos le 2026-08-30 touchait les fenêtres de *déclaration*, pas celles de *résolution* — vérifié, ce n'est pas le même rework). **Grenades bloquées** : le catalogue réel a **15 lignes `ref_equipment` category Grenade** (pas 8 comme une version antérieure du plan l'affirmait) — 7 nuages volumétriques hors périmètre, 2 à dégression standard (concussion + fragmentation), 4 à rayon fixe documenté, 2 (neuro-charge, sonique) sans aucune donnée de zone dans le RAW transcrit, page du Livre de Base à fournir par Saar. | **Seule étape non bloquée restante : §8 point 9, UI de ciblage** (mode "placer un point"/"choisir une direction", fusil à pompe/tir de suppression) — fermerait la boucle "aucun scénario réel exécuté" et rendrait le fusil à pompe jouable de bout en bout. Tireur PJ et grenades restent des blocages externes (rework non planifié / RAW manquant), pas des tâches à reprendre en l'état |
