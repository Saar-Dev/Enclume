...
| Interactions d'entité (moteur générique : déplacer un objet pour se couvrir, leviers/consoles) | — (pas de PLAN — moteur déjà construit) | Moteur générique confirmé construit et câblé (`server/src/socket/socketEntity.js`, `client/src/pages/SessionPage.jsx`, `client/src/components/EntityBuilderTab.jsx`). **Premier contenu réel livré et validé jeu réel le 2026-09-16** : les 10 assets de `futuristic_crates_chests` sont ouvrables/fermables (`docs/JOURNAL8.md`, `docs/Old/PLAN_CAISSES_INTERACTIVES.md`) — le sous-type état/animation (`target_state_id`) du moteur est donc désormais prouvé avec du contenu réel, pas seulement lu dans le code. **Le sous-type déplacement (`move_type`, pousser/tirer un objet pour la couverture) reste non prouvé** : toujours zéro blueprint avec une interaction de ce type en base. Ascenseur déjà fonctionnel (système dédié séparé, `worldElevatorService.js`). Porte : l'**interaction logique** (ouvrir/fermer, Test si besoin) est livrée et validée en session le 2026-09-02 (`.claude/rules/world.md` § « Interaction runtime sur une porte »), corrigée le 2026-09-15 (portée). Le **rendu visuel** de l'état de porte a sa propre ligne ci-dessous (§2, nouveau). La **Serrure** RAW n'est pas implémentée (ligne dédiée ci-dessous) | Créer un blueprint avec une interaction `move_type` et vérifier en jeu réel que déplacer l'objet affecte bien la couverture/LOS (`state_cover`) — dernier sous-type du moteur jamais prouvé avec du contenu. Séquencement de ce lot et des suivants : `PLANS/PLAN_ENTITES_INTERACTIVES_ROADMAP.md` (Lot A2) |
...
| **Fusion de la détection de clic 3D** (tokens/entités/connecteurs) | `PLANS/PLAN_CLIC_3D_UNIFICATION.md` (**stub 2026-09-17 — cadrage non commencé**) | Déclencheur : un token proche d'une caisse recevait le clic de la caisse même en cliquant exactement sur le token — deux systèmes de détection indépendants (boucle manuelle `Canvas3D.jsx` pour les tokens, `onClick` React Three Fiber pour les entités `EntityMesh.jsx`), sans arbitrage commun. Correctif intérimaire posé (le token participe désormais à l'arbitrage R3F). Le cadrage doit recenser tous les types d'objets cliquables (tokens, entités ×2 rendus, connecteurs, voxels) et définir un raycast unique à ordre de priorité explicite, cohérent avec le patron déjà validé dans la boucle manuelle — migration à risque, prévoir un test dédié par pack d'assets. |
