...
| Refonte design des fenêtres de déclaration de combat | `PLANS/PLAN_RW_DECLARE_DESIGN.md` | Spec écrite + révisée après analyse à charge (2026-08-28, §11 du plan). Maquette validée sur le fond avec Saar (5 itérations, `claude.ai/code/artifact/3b8fb52d-aa6c-4905-a0d1-d6712c8c44d7`). 15 décisions : une seule structure visuelle PJ/MJ/Drone/Exo (pilotage non partagé — MJ garde navigation séquentielle + preview), accent par famille (`--combat-accent-*`), l'arme = l'action (liste groupée Distance/Contact), 2 colonnes hiérarchiques, satellite « Statut », pied double-bouton, terminologie Tir/Corps à corps · Distance/Contact (`VOCABULARY.md` V2.6). Débloque le module 5 de `PLAN_RW_DECLARE_WINDOWS` (clos) et les sélecteurs d'état exo (§1 Exo-armures point 4) | **À faire maintenant** : lot B (B1-B4 : i18n + fautes d'accent + terminologie valeur JSON y compris résolution + pastille INI, un fichier à la fois) puis module 1 (réconciliation tokens `--combat-*`) — tous revert-safe. **Chantier isolé** : B5 (« Passer le tour » toujours déclarable = changement de comportement, analyse à charge dédiée). **Gaté** : modules 2-5 (chrome partagé → satellite → liste d'action → pied) ne démarrent **qu'après le module 0** = ajout d'une infra de test composant + tests de caractérisation (INFRA-4 : code combat le plus joué, le moins couvert). 7 points ouverts §7 |
