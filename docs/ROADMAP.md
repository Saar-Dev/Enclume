...
| Refonte des fenêtres de déclaration de combat (design + technique associée) | `PLANS/PLAN_RW_DECLARE_DESIGN.md` (tableau de bord §0) | **Codé + committé** (non poussé) : Lot B, B5, module 0 M0.0-M0.3 ; **module 5** (pied `CombatDeclareFooter`) ; **module 3** (satellite `CombatDeclareStatePanel`) ; **module 2 volet visuel** (re-skin teinte Wizard) — tous validés navigateur. **2026-08-30 : M0.4** (`assaultDeclaration.js`/`meleeDeclaration.js` reducers purs + hooks, câblés **PJ + MJ** — 26 tests, golden masters 67) et **module 4** (`CombatDeclareActionList` « l'arme EST l'action » : move-line + liste d'armes groupée, mode de tir en col. 2, Recharger = ↻ sur l'arme, silhouette `human.svg`) — **PJ + MJ codés et validés navigateur** | **Reste** : (1) migrer `CombatExoActionWindow` sur `CombatDeclareActionList` ; (2) retirer le sélecteur d'arme redondant de `MeleeCombatPanel` (4c) + neutraliser les couleurs de sélection inline `#5b8dee`/rouges/verts → `--decl-*` (D4b) ; (3) M0.4-f (reset consolidé) / M0.4-g (unification Charge) — différés ; (4) module 2 volet structurel (`CombatDeclareFrame`) — non prioritaire ; (5) M-E2E — non fait (golden master + validation Saar ont suffi pour le module 4). Détail : plan §0 |
