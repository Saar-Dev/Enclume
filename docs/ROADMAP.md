...
| Mécanique de point de Chance | `PLANS/PLAN_CHANCE.md` (v2.0) + `MANUELS/MANUEL_CHANCE.md` (v1.0) + `REGLES/REGLE_CHANCE.md` | **L1→L3e-4 codés et VALIDÉS JEU RÉEL 2026-09-12** (`JOURNAL8.md`). `char_sheet.chc` est directement la réserve dépensable, aucune migration de schéma pour la réserve elle-même (migrations 336-340 = machinerie `pending_chance_choices`). Régénération sur Catastrophe câblée sur les 7 sites combat identifiés (`exo_stand_up`/`exo_assault`/`exo_melee`/`assault`/`melee_defense`/`melee_attack` ; `drone_attack` exclu, pas de `char_sheet`) — choix joueur gagner/relancer posé avant résolution, garde-fou d'annulation MJ, fenêtre unique Catastrophe+Chance (une seule position à l'écran, filtrée par audience). | L4 forçage AOE, L5 réduction de gravité Blessures, L6 UI `<ChanceSpendButton>` (`PLAN_CHANCE.md` §6-8) — non commencés. Débloque le tir de suppression et lève l'écart RAW du Test de Chance AOE longue/extrême portée une fois L4 fait |
