...
interactions (`PLANS/PLAN_DIFFICULTE_INTERACTIONS_ENTITES.md`, stub, cadrage non commencé, ligne
dédiée ci-dessous — bloquant réel avant toute utilisation en jeu). Ascenseur déjà fonctionnel (système dédié séparé, `worldElevatorService.js`). Porte : l'**interaction logique** (ouvrir/fermer, Test si besoin) est livrée et validée en session le 2026-09-02 (`.claude/rules/world.md` § « Interaction runtime sur une porte »), corrigée le 2026-09-15 (portée). Le **rendu visuel** de l'état de porte a sa propre ligne ci-dessous (§2, nouveau). La **Serrure** RAW n'est pas implémentée (ligne dédiée ci-dessous) | Lots B/C de `PLANS/PLAN_ENTITES_INTERACTIVES_ROADMAP.md` restent en attente d'une décision produit (quels packs étendre) — pas bloqués techniquement, mais Déplacer n'est réellement utilisable en jeu qu'une fois `PLAN_DIFFICULTE_INTERACTIONS_ENTITES.md` cadré. |
| **Difficulté et surcharge MJ des interactions d'entité** | `PLANS/PLAN_DIFFICULTE_INTERACTIONS_ENTITES.md` (**stub 2026-09-17 — cadrage non commencé**) | Déclencheur : Déplacer une caisse avec FOR 18 (max humain) donne 15 % de chances de réussite (AN+3, Difficulté 0) — testé deux fois, deux échecs. La formule Attribut est RAW (table p.114) ; la Difficulté 0 n'est pas un choix, c'est une absence de donnée (aucun `difficulty_dc` renseigné sur aucune des 15 caisses, RAW silencieuse sur la poussée d'objet). Aucune interface MJ n'existe pour surcharger une Difficulté au cas par cas (`EntityInstancePanel.jsx` n'a jamais eu ce champ), pour aucune interaction d'entité — pas seulement Déplacer. Bloquant réel avant utilisation en jeu de Déplacer. |
...
| **Empêcher de poser un token sur une case déjà occupée** | `PLANS/PLAN_BLOCAGE_CASES_OCCUPEES.md` (**stub 2026-09-17 — cadrage non commencé**) | Déclencheur : après le correctif d'occupation circulaire (`spatialIndex.js`), un PNJ s'est révélé réellement immobilisé parce qu'il occupait la même case qu'une caisse — pas un bug, mais rien n'empêche aujourd'hui de poser un token sur une case déjà occupée à la pose initiale (`canOccupy` n'intervient qu'au déplacement). Décision produit à trancher avec Saar (refus dur / avertissement / snap automatique) avant tout code ; recenser d'abord tous les flux de pose de token. |
...
| Drones — mode Autonome (Sprint 2d) et Télépilotage (Sprint 3) | `PLANS/PLAN_DRONE.md` (**audit complet du code réel 2026-09-16 — remplace les PLAN archivés `docs/Old/PLAN_DRONE.md`/`PLAN_DRONESYSCOMBAT.md`, périmés depuis la refonte échelle de phases Session 159**) | Fiche drone + combat manuel (Sprint 1/1bis/2a/2b/2c) **clos et vérifiés en code**. Les 2 bugs listés ici jusqu'au 2026-09-16 (budget de mouvement drone, CaC drone sans branche `isDrone` à l'annonce) sont **déjà corrigés**, retirés de ce document. Reste : Sprint 2d (mode autonome RAW — Détection→Ami/Ennemi→Armement sans intervention, aucun drone n'est aujourd'hui autonome, tous se déclarent manuellement comme un PNJ) et Sprint 3 (télépilotage — rien codé, compétence `TELEPILOTAGE` déjà au catalogue). Le redesign de 2d contre l'architecture actuelle (`combat_timeline_entries`) est à cadrer avant tout code — l'ancien pseudo-code (`active_slot_idx`/`sequence`) ne s'applique plus |
