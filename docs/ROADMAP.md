...
| Encyclopédie RAW (fenêtre de consultation Polaris, Livre 4) | `PLANS/PLAN_ENCYCLOPEDIA.md` | Premier commit posé le 2026-09-22 (`9155c39`). Tests et actions (8/8), Combat (20/20), États de santé (25/25, segmenté 31→25) clos et vérifiés, toutes tables RAW connues branchées sur `shared/`. Force Polaris (12/13 articles) audité 2026-09-22 : 1 fichier corrompu (`maitrise-force-polaris.json` contient en réalité le texte de « Choc Polaris ») + `liste-pouvoirs.json` (44 pouvoirs, 1350 lignes) à segmenter comme États de santé l'a été — **8 blocs `dataTable` invisibles (schéma éditorial jamais supporté par `DataTableBlock.jsx`) corrigés le jour même**. Expérience (0 article) et Livres 1-2-3 (texte non communiqué) non commencés. **Aucun point d'entrée dans la nav de l'app** (`bug_tickets` `ENCYCLOPEDIE-NOT-INTEGRATED`, priorité low) | (1) obtenir le RAW p.252 pour réécrire `maitrise-force-polaris.json` ; (2) décider la segmentation de `liste-pouvoirs.json` ; (3) chapitre Expérience ; (4) intégration dans la nav de l'app |
...
|---|---|---|
| Forme de collision des entités (cercle → forme explicite) | `PLANS/PLAN_FORME_COLLISION_ENTITES.md` (**v3, deux analyses à charge indépendantes passées, prête à coder**) | Root cause vérifiée sur données réelles : `entityOccupant` modélise toute entité comme un cercle (`max(width,depth)/2`), faux pour un objet allongé (pack de caisses → rayon 1,13 m calculé, bloque un token à 1 m de son côté étroit). Design : `collider.shape` explicite (`circle`/`rect`, capsule notée V2), défaut `rect` pour le catalogue actuel, rotation `entity.r` prise en compte (formule corrigée en revue : 0-3/90°, pas 0-7/45°). Rien codé — chantier lourd (touche l'autorité de collision partagée par tout le moteur monde), en attente de démarrage. |
