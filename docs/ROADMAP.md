...
| Refonte des fenêtres de déclaration de combat (design + technique associée) | `PLANS/PLAN_RW_DECLARE_DESIGN.md` (tableau de bord §0) | **Codé + committé** (non poussé) : Lot B, B5, module 0 M0.0-M0.3 ; **module 5** (pied `CombatDeclareFooter`) ; **module 3** (satellite `CombatDeclareStatePanel`) ; **module 2 volet visuel** (re-skin teinte Wizard) — tous validés navigateur. **2026-08-30 : M0.4** (`assaultDeclaration.js`/`meleeDeclaration.js` reducers purs + hooks — 26 tests, golden masters 67) + **module 4** (`CombatDeclareActionList` « l'arme EST l'action » : move-line + liste d'armes groupée, mode de tir en col. 2, Recharger = ↻ sur l'arme, silhouette `human.svg`) câblés **PJ + MJ + Exo** ; **4c** (sélecteur d'arme redondant de `MeleeCombatPanel` retiré) ; **D4b** (couleurs de sélection col. 2 → `--decl-acc`). **Fonctionnellement clos** — validé navigateur | **Reste — tout différé / non prioritaire** : M0.4-f (reset consolidé) / M0.4-g (unification Charge) ; module 2 volet structurel (`CombatDeclareFrame`, jugé secondaire par `REACT.md` P58) ; M-E2E (golden master + validation Saar ont suffi). Archiver le plan sur confirmation Saar. Détail : plan §0 |
