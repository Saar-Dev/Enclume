...
| Mécanique de point de Chance | `PLANS/PLAN_CHANCE.md` (v2.0) + `MANUELS/MANUEL_CHANCE.md` (v1.0) + `REGLES/REGLE_CHANCE.md` | **Cadrage terminé 2026-09-11** (RAW fourni par Saar, transcrit — le blocage précédent est levé). `char_sheet.chc` est directement la réserve dépensable, aucune migration nécessaire. V1 : Test de Chance générique, forçage combat (Événement favorable), réduction de gravité des Blessures, régénération sur Catastrophe (câblage système entier — pas limité au combat —, choix joueur gagner/relancer, garde-fou d'annulation MJ sur le patron déjà validé `RepairRequestCard.jsx`), bouton narratif minimal (Indice / Événement favorable narratif, zéro résolution automatisée). Coup de pouce (+5, marge forcée à 0) reporté V2. Mort subite et Maladies/poisons/drogues/irradiations hors périmètre — RAW complet mais dépendances externes absentes du projet (mécanique de mort, domaine « États de santé ») | L0→L6 dans l'ordre (`PLAN_CHANCE.md` §10) — L0 vérification seule (aucune migration), L1 primitive partagée + bascule Petit bouclier, L2/L3 services, L4 forçage AOE, L5 réduction de gravité, L6 UI. Débloque le tir de suppression et lève l'écart RAW du Test de Chance AOE longue/extrême portée |
...
| Usure & Intégrité — **Lot 2 uniquement** (V1 L0→L7 clos et poussé 2026-09-10, `docs/JOURNAL8.md`) | `PLANS/PLAN_USURE&INTEGRITE.md` §10 + `MANUELS/MANUEL_USURE.md` v1.5 | **V1 livrée** : schéma (migrations 329-335), `shared/integrityRules.js` + `integrityService.js`, modificateur/panne d'arme en combat, réparation (échéance `equipment_repair` + carte d'action chat + pop-up ITG). **Reste le Lot 2** : **L8** « taper dessus » + pièces détachées → dépend du helper `resolveChanceTest` (chantier Chance ci-dessus §1 — **cadrage terminé, prêt à coder**, n'est plus bloqué par le RAW) ; **L9** entrées #2 (`armeInutilisable`) / #8 (`panneSysteme`) de la table Catastrophe combat → peupler `EFFECT_HANDLERS` de `catastropheService.js` (cadré : `PLAN §10.2` + `MANUEL_USURE.md` §8 ; prérequis doc : corriger `COMBAT.md` « conséquences narratives » périmé + `ROADMAP.md` ci-dessous) |
