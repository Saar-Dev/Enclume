...
|---|---|---|
| Armes spéciales (fusil à pompe, lance-flammes, grenades/mines) | `PLANS/PLAN_ARMES_SPECIALES.md` | **Plan rédigé 2026-09-03** (recherche code + RAW faites). Prérequis AOE levé — fusil à pompe clos (PNJ + PJ). **Lot 1 — lance-flammes : cadré, pas commencé** — géométrie `cone` (`aoeShapes.js`) + exclusivité (AOE étape 7) + `armorReductionFactor: 0.5` (protections ÷2) + `exposeToHazard('burning')` (feu continu, Fatigue&Dommages Lot 3) existent déjà ; à écrire = prédicat partagé `isAoeWeapon`, aperçu cône, branche resolveur (1D3 Localisations, Choc 2D6, pas de dégression par portée), param `expiresAtTurn` sur `exposeToHazard`. **4 décisions RAW à valider avec Saar** avant code (dimensions du cône, auto-éclaboussure < 3 m, Choc sur toutes les cibles, armures « simples »). **Lot 2 — grenades : bloqué** (migration catalogue §6c + champ payload `intendedOrigin` + infra d'action différée inter-tours + 2 pages RAW neuro-charge/sonique à fournir par Saar). Mines : hors scope v1. Fouets/chaînes : → Arts martiaux. |
| **Résolution de zone d'effet (AOE)** | `PLANS/PLAN_AOE.md` (v11, §12 tient l'avancement réel à jour) | **Rafraîchi 2026-09-03 (analyse code, pas déduit).** **CLOS** : **fusil à pompe jouable de bout en bout, tireur PNJ ET PJ** (validé session réelle Saar). PNJ : étapes 1-9, commit `117b18a`. **Tireur PJ : clos 2026-09-03** (§8 étape 10 + JOURNAL8) — deux plans intermédiaires écartés après conception ((1) « rework de séparation des fenêtres » : dépendance inexistante ; (2) « N `armAwaitingDamage` FIFO » : le pipeline différé + le hook client supposent 1 cible, N pending d'un seul appel corrompt l'UI). Design retenu : résolution immédiate, tireur PJ = même boucle que le PNJ + un `COMBAT_ATTACK_PLAYER_RESULT { targets: [...] }` agrégé, liste par cible dans `CombatModifiersWindow` ; ne touche aucun code différé partagé. **Tir de suppression — doublement bloqué**, plus lourd qu'il n'y paraît : (i) toute la résolution repose sur un Test de Chance, absent du schéma (chantier Chance différé, §4) ; (ii) c'est une zone **persistante inter-tours** qui contraint le déplacement — il faut un objet zone vivant dans `combat_state` que `planCombatWorldMovement` consulte (les zones AOE actuelles sont ponctuelles). Attend le chantier Chance a minima. **Grenades bloquées** : migration catalogue §6c + champ payload `intendedOrigin` (lancer un point, déviation 1D6 à l'échec, explosion au Tour suivant au rang d'Ini) + 2 types sans donnée de zone dans le RAW transcrit (neuro-charge, sonique — pages Livre de Base à fournir par Saar). | Prochain pas AOE : **lance-flammes** (ligne Armes spéciales). Le fusil à pompe est jouable de bout en bout (PNJ + PJ). Grenades / suppression restent des blocages, pas des tâches à reprendre en l'état |
