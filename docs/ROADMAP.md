...
| Informatique (ordinateurs + pannes électroniques + IEM) | `PLANS/PLAN_INFORMATIQUE.md` + `MANUELS/MANUEL_INFORMATIQUE.md` | **Lot 1+2 VALIDÉS JEU RÉEL, committés/poussés (`64c6667`..`daa4e4a`).** **Lot 2bis (ciblage IEM exo-armure) CODÉ ET VALIDÉ JEU RÉEL 2026-09-16** (Armement + Systèmes auxiliaires rejoués en combat, Exosquelette/Générateur acceptés sur preuve des tests). **Lot 3b (Survie I.E.M., machine à états) CODÉ 2026-09-16, pas encore validé en jeu réel** : analyse critique avant code a trouvé que `runIemPanneTriggerExo` (Lot 2bis) ne pouvait jamais atteindre `exo_computers` — fixé (pool fusionné avec l'ordinateur ACTIF, jamais les deux — collision `UNIQUE(token_id, status_code)` sinon). Nouveau fichier `iemSurvivalService.js` (pose+tick), migration 353 (`sequelle_malus`), 4ᵉ source `activeMalusRegistry`, extension additive `combatantContextService`, garde de blocage `socketCombatAnnouncement`. 112/112 tests verts. Gap satellite non traité (hors périmètre) : un exo/drone tireur ne peut charger aucune munition typée. Lot 4 (auto-désactivation Gestion systèmes) cadré, indépendant, pas codé | Ordre restant : validation jeu réel du Lot 3b (Saar) → commit/push → Lot 4 (indépendant, peut se faire avant ou après). Détail architecture à jour dans `PLAN_INFORMATIQUE.md` §4 |
