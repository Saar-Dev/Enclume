...
| Informatique (ordinateurs + pannes électroniques + IEM) | `PLANS/PLAN_INFORMATIQUE.md` + `MANUELS/MANUEL_INFORMATIQUE.md` | **Lot 1+2 VALIDÉS JEU RÉEL, committés/poussés (`64c6667`..`daa4e4a`).** **Lot 2bis (ciblage IEM exo-armure) CODÉ ET VALIDÉ JEU RÉEL 2026-09-16.** **Lot 3b (Survie I.E.M., machine à états) CODÉ 2026-09-16, testé par trace de code (chemin RNG à faible probabilité), pas encore observé en jeu réel de bout en bout** : analyse critique avant code a trouvé que `runIemPanneTriggerExo` (Lot 2bis) ne pouvait jamais atteindre `exo_computers` — fixé (pool fusionné avec l'ordinateur ACTIF, jamais les deux — collision `UNIQUE(token_id, status_code)` sinon). Nouveau fichier `iemSurvivalService.js` (pose+tick), migration 353 (`sequelle_malus`), 4ᵉ source `activeMalusRegistry`, extension additive `combatantContextService`, garde de blocage `socketCombatAnnouncement`. 112/112 tests verts. **Lot 4 (auto-désactivation Gestion systèmes) CODÉ ET VALIDÉ JEU RÉEL 2026-09-16** (Saar : « Fonctionnel ») : fonction pure `shared/exoSystemsCapacity.js` (8/8 tests), enrichissement GET `/exo/systems`, exclusion des systèmes déconnectés du tirage IEM (« hors service, rien à griller »), boutons ↑/↓ de réordonnancement (pas de drag&drop — le patron supposé existant dans 3 autres panneaux s'est révélé faux, décision d'aggradation). Gap satellite non traité (hors périmètre) : un exo/drone tireur ne peut charger aucune munition typée | Ordre restant : validation jeu réel du Lot 3b (Saar, dernier point ouvert du chantier) → commit/push. Détail architecture à jour dans `PLAN_INFORMATIQUE.md` §4 |
...
| Serrures (contrôle d'accès sur porte) | `REGLES/REGLE_SERRURE.md` (RAW transcrite 2026-09-10, **aucun PLAN écrit**) | Rien cadré ni codé — l'interaction porte actuelle (§1, Interactions d'entité) ne distingue aucun type de serrure (optique/digital/ADN/vocal, coûts x2 à x5) ni le fusible (3 tentatives, +20% coût). Lié à `REGLE_ORDINATEUR.md` (neutralisation par ordinateur, Duel d'ordinateurs) — dépend probablement du chantier Informatique ci-dessous (flag `is_electronic`, primitive de Test déjà existante côté Usure) pour la neutralisation active, indépendant pour la reconnaissance passive (code/carte/biométrie) |
