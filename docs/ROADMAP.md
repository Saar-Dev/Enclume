...
| Refonte des fenêtres de déclaration de combat (design + technique associée) | `PLANS/PLAN_RW_DECLARE_DESIGN.md` (tableau de bord §0) | **Codé + committé** (non poussé) : Lot B (i18n + terminologie Tir/CaC + pastille INI), B5 (« Passer le tour » mécanisme), module 0 M0.0-M0.3 (`buildDeclarePayload` + 51 tests). **Cadré + analyse à charge** : modules 2 (châssis `CombatDeclareFrame`), 3 (satellite d'état), M0.4 (hooks `useAssaultDeclaration`/`useMeleeDeclaration`), 4 (liste d'action « l'arme EST l'action »), 5 (pied). Prototype D5 validé (`claude.ai/code/artifact/afcd5e28-341b-40ee-b109-30e69d9597fc`). Teinte Wizard (P7). Analyse à charge profonde faite (§19) puis **plan réécrit-synthétisé** (2090→675 l., journal archivé `docs/Old/PLAN_RW_DECLARE_DESIGN_JOURNAL.md`) ; **ordre B tranché** (§5.0) ; **M-E2E = filet local** tranché (§5.5) | **Coder** (conversation à part), ordre B : module 5 (pied autonome) → module 3 (satellite autonome) → **M-E2E** (harness Playwright local, `POST /api/test/combat-fixture`) → module 2 (frame, re-slot) → M0.4 → module 4 (4a-4e). Validation navigateur Saar après **chaque sous-module** (R2). Détail : plan §5.0 |
