...
| **Fondation « zones dangereuses persistantes »** | `PLANS/PLAN_ZONES_DANGER.md` (**cadrage rédigé 2026-09-09**) | **Trouvaille 2026-09-09** : `world_effect_instances` + `shared/world/worldEffects.js` = échafaudage. Fonctionne : modèle de données volumique, définitions builtin (`fire`/`gas`/`flooded`/`oil`/`unstable`), coût de déplacement à travers, occlusion LOS. **Ne fonctionne PAS** : application de dégâts (ni traverser — events calculés/persistés, aucun `resolveTargetHit` —, ni `turnStart` — aucune boucle), `duration_rounds` jamais décrémenté, rien ne crée d'instance depuis le combat. Cadrage : §1 constat `[VÉRIFIÉ]` ; périmètre réduit au **noyau** (boucle de Tour + expiration + pont hook→résolveur unique + `createWorldEffectInstance` depuis le combat), consommateur-preuve = **grenade incendiaire** ; hors périmètre = 0G/vide/escalade/pièges/eau montante. RAW feu+acide+décompression+noyade+froid+radiations transcrit ; réfs pro Foundry v12 / PF2e. **Débloque** : grenade incendiaire + gaz/fumigène (`PLAN_NUAGE.md`) + capsules + **tir de suppression** (zone persistante = 1 de ses 2 bloqueurs) + zones dangereuses MJ. **§4 (liste de champs) NON FIGÉE — à construire cas par cas avec Saar avant tout code.** |
