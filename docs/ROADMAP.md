...
|---|---|---|---|
| Exo-armures (v2) | `PLANS/PLAN_EXOARMURE.md` | **§16.3 Étape A (déplacement) et §16.4 Étape B (Tir/CaC) codés et validés en combat réel le 2026-08-27** — premier combat exo-armure fonctionnel du projet (Tir qui touche, résolution complète, étourdissement de la cible, échange normal dans les deux sens). Détail mécanique : `docs/SYSTEME/EXOARMURE.md` §5. Lots 1-4 + 2bis + §16.2.1/2.2/2.5 déjà codés et testés antérieurement. `ExoSheetWindow.jsx` fonctionne | Points ouverts, non bloquants pour jouer : (1) **[FAIT 2026-08-27]** Compétence Tir Automatique (PC23) exo RC/RL — règle identique à l'humanoïde, gate + message explicite dans `socketCombatAnnouncement.js` branche `isExo`, cf. `docs/SYSTEME/EXOARMURE.md` §5 ; (2) Initiative projetée près de DÉCLARER + (4) rework UI fenêtre d'action + statuts manquants (arme rangée/au clair, position accroupi/genou) → **absorbés par `PLANS/PLAN_RW_DECLARE_WINDOWS.md`** (item 2 = lot 3 de ce chantier ; statuts = hors-scope V1 mais cadrés dessus) ; (3) combat drone jamais retesté en jeu réel depuis le refactor DRY de `resolveDroneAssaultAction` (2026-08-26, extraction pure non prouvée) — Saar teste |
| RW fenêtres de déclaration de combat (briques partagées + 3 orchestrateurs) | `PLANS/PLAN_RW_DECLARE_WINDOWS.md` | **v2 écrite 2026-08-27** — v1 abandonnée (rédigée sur lecture incomplète, contredisait REWORK-05). v2 sur lecture intégrale des 3 fenêtres + docs, suit `METHODO_PLAN.md`. Approche actée avec Saar : « finir REWORK-05 » — extraire les morceaux de châssis encore dupliqués (`StateSelector`, widget Initiative, bandeau d'erreur, roster, chrome) en **briques partagées**, **un module à la fois**, fini+validé navigateur avant le suivant ; **on garde 3 orchestrateurs séparés** (fusion GM+Joueur rejetée par REWORK-05). Porte l'item 2 exo (initiative projetée = module 2). Pur refactor client, zéro changement DB/REST/WS. 6 modules listés §5, du plus sûr au plus délicat ; modules 5-6 (chrome, `useHumanDeclare`) re-décidés après 1-4 | **Module 1 FAIT + validé (2026-08-28)** : `StateSelector` → `client/src/components/CombatDeclareStateSelector.jsx` (`export default`, convention `CombatDeclare*` fixée, D7). En attente de commit (groupé). **Ordre révisé après analyse à charge du cadrage module 6** : fix Tir visé (correctif ciblé, §5bis) → module 2 (item 2) → 3 → 4 → re-décider module 6 (prérequis vitest + tests de caractérisation) → module 5 (probable annulation). Le module 6 (`useHumanDeclare`) est reporté : le bug Tir visé se corrige sans extraire, INFRA-4 déconseille le découpage proactif, et sans tests de caractérisation l'extraction = bug de combat garanti. **Prochain : plan du correctif Tir visé** (re-sync état de déclaration au nouveau tour — `declarationReducer.js` + effets de reset des 2 fenêtres) |
