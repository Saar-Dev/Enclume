...
|---|---|---|---|
| Exo-armures (v2) | `PLANS/PLAN_EXOARMURE.md` | Lots 1-4 + 2bis codés et testés (Node/PostgreSQL). §16.2.1 (plafond Manœuvre d'armure généralisé), §16.2.2 (armures assistées) et §16.2.5 (milieu hybride manuel, sans repli automatique) codés et testés le 2026-08-25 (39/39 + 336/336, contre PostgreSQL réel). `ExoSheetWindow.jsx` existe et fonctionne (ticket `ARMORWINDOW-MISSING1` fermé — était périmé, la fiche existe depuis le commit `40c8231`). **Priorité confirmée par Saar (2026-08-25)** : aujourd'hui, une exo-armure n'a strictement aucune action possible en combat (ni déplacement, ni attaque, ni autre) — la fenêtre d'action passe avant Usure/Intégrité et Informatique/pannes (§2), qui *complètent* Exo-armures plutôt que de la rendre jouable | §16.3 Étape A (déplacement combat exo) et §16.4 Étape B (attaque Tir/CaC) — dépendent de §16.2.3 (migration `exo_weapons.ammo_remaining`) et §16.2.4 (seed `skill_id`/`ammo_count`, 13 armes), deux tâches indépendantes, faisables en parallèle |
...
| Portes (connecteurs) | `PLANS/PLAN_INTERACTIONS_CONNECTEURS.md` | N'est pas un plan — une base de recherche. Portée réduite depuis la re-vérification du 2026-08-25 (voir §1, "Interactions d'entité") : seules les portes/échelles (`surface_data.connectors`) restent un vrai gap technique, tout le reste du bucket "interactions environnement" est soit déjà fait soit déjà câblé. **Décision d'architecture nécessaire avant de cadrer** : bâtir un système bespoke pour les portes (mirroir du pattern ascenseur, `worldElevatorService.js`) ou les faire passer par le moteur d'interaction générique déjà existant pour les entités (§1) ? Origine : ticket `ENTITYCLICK1` |
