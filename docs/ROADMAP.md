...
| Refonte design des fenêtres de déclaration de combat | `PLANS/PLAN_RW_DECLARE_DESIGN.md` | Spec écrite + révisée après analyse à charge (2026-08-28, §11 du plan). Maquette validée sur le fond avec Saar (5 itérations, `claude.ai/code/artifact/3b8fb52d-aa6c-4905-a0d1-d6712c8c44d7`). 15 décisions : une seule structure visuelle PJ/MJ/Drone/Exo (pilotage non partagé — MJ garde navigation séquentielle + preview), accent par famille (`--combat-accent-*`), l'arme = l'action (liste groupée Distance/Contact), 2 colonnes hiérarchiques, satellite « Statut », pied double-bouton, terminologie Tir/Corps à corps · Distance/Contact (`VOCABULARY.md` V2.6). Débloque le module 5 de `PLAN_RW_DECLARE_WINDOWS` (clos) et les sélecteurs d'état exo (§1 Exo-armures point 4) | **Lot B codé (2026-08-28)** : `combat.json` (accents + terminologie « Tir »/« Corps à corps » y compris résolution + pastille `INI = 7 -> 2`) + `CombatGmDeclareWindow` (`DÉCLARER` i18n) + `socketCombatAnnouncement` (swap « Assaut »→« Tir »). Build propre, validation navigateur en bloc à venir. **Suite** (round 4, priorités Saar « qualité structurelle >>> vitesse, rework pour stabiliser ») : **B5** (chantier isolé) → **module 0** = extraire `buildDeclarePayload`/`useHumanDeclare` en modules purs `.mjs` + tests de caractérisation (golden master, philo `node --test` du projet, pas de vitest) → **modules 2→5** (chrome partagé + accent par famille → satellite → liste d'action → pied), un validé avant le suivant. Module 1 (tokens) fusionné dans le 2. 7 points ouverts §7 |
