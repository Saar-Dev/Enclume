...
| Informatique (ordinateurs + pannes électroniques + IEM) | `PLANS/PLAN_INFORMATIQUE.md` + `MANUELS/MANUEL_INFORMATIQUE.md` | **Plan complet, 2026-09-15 — tous les points bloquants tranchés avec Saar, exécution non commencée (aucun code appliqué).** Ciblage IEM par catégorie de porteur, Repository pattern pour `runPanneTest` partagé `char_inventory`/`exo_computers`, critère Gestion systèmes (premier branché/débranché), et — dernier point tranché — le blocage d'action d'un pilote d'exo-armure pendant l'immobilisation Survie I.E.M. (opérateur de drone jamais concerné). Survie I.E.M. scindée en Lot 3a (ressource `survie_iem_max/current`, migration 345 écrite) et 3b (machine à états immobilisation/redémarrage/séquelle, architecture posée, pas encore codée). Migrations 344/345 + route `char-sheet.js` (Lot 1/3a) écrites et vérifiées, **non appliquées** | Écrire et tester le code des 4 lots dans l'ordre : Lot 1 (catalogue `is_electronic` + seed) → Lot 2 (déclencheur Test de panne IEM, refactor Repository) → Lot 3a/3b (Survie I.E.M.) → Lot 4 (auto-désactivation Gestion systèmes) — puis `db.migrate.latest()` |
