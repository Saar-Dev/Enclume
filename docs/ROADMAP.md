...
| **Fondation « zones dangereuses persistantes »** | `PLANS/PLAN_ZONES_DANGER.md` (**réécrit propre 2026-09-10 — cadrage terminé, prêt à coder sur validation Saar**) | Deux demi-systèmes (`world_effect_instances`/`worldEffects.js` spatial mais inerte ; système « dangers environnementaux » Lot 3 — `environmentalHazardRegistry`/`Presets`/`Service` + `TokenStatusPanel` — qui tick mais n'a rien de spatial). **Architecture** : `effectLineResolverRegistry` unique (patron `resolveModHooks` = `GameplayEffectComponents` GAS) · **refonte** du système Lot 3 (presets absorbés dans `dangerCatalog.js`, registre dérivé, tick codé en dur remplacé par le dispatch) · `dangerCatalog.js` = bible RAW sourcée · `puissance` additif · `protections` JSONB · interaction par tags. Patron **spawner** · **hors-combat HORS SCOPE**. **Plan Z0→Z7, noyau Z0→Z5** ; éditeur E-v1 (rectangle + « remplir un compartiment ») = Z6 ; sculpteur polygone E-v2 = après le rework world builder (§12). Décisions Saar A→G toutes tranchées (§8). **Débloque** : grenade incendiaire + gaz/fumigène (`PLAN_NUAGE.md`) + capsules + tir de suppression + zones MJ. v2 différé : `test`/`statLoss`/`chance`/Souffle/`accumulateLevel` (radiations)/`corrodeEquipment`/`chain`/animation géométrique/interaction zone×zone/pièges. |
