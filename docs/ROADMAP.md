...
| Mécanique de point de Chance | `PLANS/PLAN_CHANCE.md` (v2.0) + `MANUELS/MANUEL_CHANCE.md` (v1.0) + `REGLES/REGLE_CHANCE.md` | **L1→L4 codés et VALIDÉS JEU RÉEL 2026-09-12** (`JOURNAL8.md`). `char_sheet.chc` est directement la réserve dépensable. Régénération sur Catastrophe câblée sur les 7 sites combat identifiés (`exo_stand_up`/`exo_assault`/`exo_melee`/`assault`/`melee_defense`/`melee_attack` ; `drone_attack` exclu, pas de `char_sheet`), fenêtre unique Catastrophe+Chance. **L4 (forçage/Test de Chance longue-extrême portée AOE)** : site `aoe_avoidance`, patron Aggregator/Scatter-Gather (jonction verrouillée `pg_advisory_xact_lock`, plusieurs cibles simultanées) — comble l'écart RAW du fusil à pompe/grenade à portée longue/extrême, validé jeu réel (course à 13ms testée). | L5 réduction de gravité Blessures, L6 UI `<ChanceSpendButton>` (`PLAN_CHANCE.md` §7-8) — non commencés. Le tir de suppression reste bloqué par sa propre dépendance (zone persistante inter-tours), pas par Chance |
...
| Usure & Intégrité — **Lot 2 uniquement** (V1 L0→L7 clos et poussé 2026-09-10, `docs/JOURNAL8.md`) | `PLANS/PLAN_USURE&INTEGRITE.md` §10 + `MANUELS/MANUEL_USURE.md` v1.5 | **V1 livrée** : schéma (migrations 329-335), `shared/integrityRules.js` + `integrityService.js`, modificateur/panne d'arme en combat, réparation (échéance `equipment_repair` + carte d'action chat + pop-up ITG). **Reste le Lot 2** : **L8** « taper dessus » + pièces détachées → dépend du helper `resolveChanceTest` (chantier Chance ci-dessus §1 — **codé et validé jeu réel**, plus aucun blocage) ; **L9** entrées #2 (`armeInutilisable`) / #8 (`panneSysteme`) de la table Catastrophe combat → peupler `EFFECT_HANDLERS` de `catastropheService.js` (cadré : `PLAN §10.2` + `MANUEL_USURE.md` §8 ; prérequis doc : corriger `COMBAT.md` « conséquences narratives » périmé + `ROADMAP.md` ci-dessous) |
| Informatique (ordinateurs + pannes électroniques + IEM) | `PLANS/PLAN_INFORMATIQUE.md` (stub, périmètre décidé 2026-09-09) | Fusion de « Informatique et pannes » et de « Mécanique IEM » (ex-backlog §4) en un seul chantier. RAW transcrite (`REGLES/REGLE_ORDINATEUR.md`), rien cadré. **Dépendance Usure levée** : la primitive de test de panne existe (`integrityService.runPanneTest` = `resolvePolarisTest` sur l'ITG ; `shared/integrityRules.js`), le flag `is_electronic` reste à ajouter côté ce chantier. **Confirmé Saar (2026-08-25) : nécessaire pour finir Exo-armures** |
