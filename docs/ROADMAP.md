...
| **6ᵉ ligne du compteur de blessures** (Mort subite / Membre détruit, seuil 30) | `PLANS/PLAN_BLESSURE_SIXIEME_LIGNE.md` (Lots 2-4) ; documentation du statut « Mort » livré : `SYSTEME/STATUTS_TOKEN.md` | **Prérequis livré et validé en jeu (2026-09-24)** : statut de token `dead` (réservé MJ), registre unique des statuts, blocage proactif (un token mort/étourdi/inconscient est passé sans fenêtre), pas de Chance ni de test de Choc pour un cadavre, purge des états de corps vivant à la mort — chantier « Statut Mort » clos et archivé (`Old/PLAN_STATUT_MORT.md`). **Lots 2-4 non commencés** : le moteur n'a que 5 lignes sur 6 (un coup ≥ 30 reste une Mortelle ; le débordement d'une ligne Mortelle pleine n'écrit aucune blessure) | **Lot 2** (migration `chk_wounds_severity`, 6ᵉ gravité, retrait de `is_lethal`, seuils uniques `woundSeverityForDamage`, débordement, la blessure « Mort » pose `dead` via `applyDeathConsequences`), puis Lot 3 (Chance à 3 points → Critique) et Lot 4 (état permanent du membre + rendu barré/gris). Plan présenté + analyse à charge avant tout code |
...
- Membres détruits (distinction Mortelle vs Membre détruit) — **cadré 2026-09-23/24** dans `PLANS/PLAN_BLESSURE_SIXIEME_LIGNE.md` (Lots 2-4, voir §1) ; jusque-là la gravité Mortelle couvre Bras/Jambes comme Tête/Corps
