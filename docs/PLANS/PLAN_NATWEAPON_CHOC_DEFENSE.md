...
| 5 sites | **Codés + commités + poussés `dev/Saar` (`b5e4ce8`)** — mise à jour 2026-09-05 : la ligne « code non commité » ci-dessous était périmée, un commit propre a été fait ensuite (message détaillé, 5 points de contact). |
| Tests + session réelle | **Bloqué — validation différée.** Tentative Saar (2026-09-05, Baboulinet/Corne vs PJ en défense) rejetée en amont par `getNaturalWeaponIneligibilityReasons` (`shared/naturalWeapons.js`, chantier `PLAN_MUTATION2` antérieur, sans rapport) : la mutation « Corne » exige `natural_weapon_requires_grapple` (cible préalablement saisie), mécanique **non implémentée**. Seule « Corne » porte une formule de Choc parmi les armes naturelles (vérifié en base) — aucune autre mutation ne permet de tester ce chemin sans Saisie. **Décision Saar : validation différée à l'implantation de la Saisie CaC** (chantier Arts martiaux / CaC avancé, `REGLECACARTMARTIAUX.md`, aucun PLAN écrit). Le code `b5e4ce8` est en place mais son comportement reste **non vérifiable** jusque-là. Chantier à rouvrir en même temps que la Saisie.
