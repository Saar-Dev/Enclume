...
>
> **Décision de séquencement (Saar, 2026-08-27) — RÉVISÉE 2× le 2026-09-03 après analyse code.**
> 1ʳᵉ révision : la v9-v10 disait « tireur PJ attend un rework de séparation des fenêtres par type de
> combattant, en cours de planification » — **cette dépendance n'existe pas** (dispatch de résolution
> déjà séparé par type `socketCombatResolution.js:402-406` ; `CombatDamageWindow` déjà PJ-only et
> agnostique ; aucun « rework de fenêtres » jamais planifié, `ROADMAP.md` §1 vérifié ; la dette
> `ROADMAP.md` §5 porte sur les *resolvers* drone, pas des fenêtres, et n'est pas un prérequis).
> 2ᵉ révision : le plan qui a suivi (tireur PJ = armer N `armAwaitingDamage` FIFO + champ
> `spreadDamageDice` dans `confirmDamage`) a été **conçu pour de bon puis écarté** — le pipeline
> différé et le hook client supposent UNE cible à la fois, N pending d'un seul appel corrompt l'UI
> (§5.1 révisé). **Design final : résolution immédiate, tireur PJ = même boucle que le PNJ + un
> `COMBAT_ATTACK_PLAYER_RESULT` agrégé** (§5.1, §8 étape 10). L'étape 9 (UI de ciblage) est
> indépendante et close ;
...
### 5.1 PJ/PNJ — orchestration, pas un nouveau pipeline de dégâts [CIBLE — révisé 2026-09-03]

**v1-v11 (écarté)** : « PJ → un `COMBAT_DAMAGE_PROMPT` par cible, les N prompts en parallèle, chacun
répondu dans `CombatDamageWindow` ». Conçu pour de bon le 2026-09-03 (tranche tireur PJ du fusil à
pompe) — **ne marche pas** : le pipeline différé **et** le hook client (`useCombatSocket.js`) supposent
UNE cible en attente à la fois. `confirmDamage` émet `COMBAT_DAMAGE_PROMPT(cible k+1)` **avant**
`COMBAT_DAMAGE_RESULT(cible k)` (`socketCombatHelpers.js:926` puis la branche cible ~:1011) ; côté
client il n'y a qu'un slot `damagePayload` + un slot `damageResults`, et `onDamagePrompt` ne nettoie
pas les résultats. Pour une AOE 3 cibles : la fenêtre affiche le nom de la cible k+1 avec la
Localisation/les dégâts de la cible k et un bouton « Fermer » qui jette les cibles restantes — serveur
bloqué en `AWAITING_DAMAGE`. (Bug latent identique pour le CaC multi-attaques sur défenseurs PJ
distincts, `resolveMeleeDefenseHitAttackerPj` — jamais exercé ; le premier « N pending armés en un seul
appel » serait cette tranche AOE.)

**Décision (Saar, 2026-09-03)** : pour une arme de zone, **le seul jet joueur qui a du sens est le jet
Phase A** (le « Lancer » déjà fait dans `CombatModifiersWindow`). Les dégâts d'une gerbe sont
`dés d'arme + MR + dé_de_réduction(palier)` × N cibles — aucun « lancer unique » satisfaisant à animer,
et faire itérer un mécanisme d'UI conçu pour 1 cible est ce qui crée tout le problème multi-pending.

**Design retenu — résolution immédiate + panneau de résultat par cible :**
- Tireur **PNJ** : boucle de résolution immédiate, inchangée.
- Tireur **PJ** : **la même boucle**, exécutée côté serveur après le jet Phase A. PJ et PNJ AOE
  deviennent le même code — plus de fan-out différé, plus de `combat_action_targets` alimenté par N
  `confirmDamage`, plus de risque de tour bloqué (§5.4 sans objet pour l'AOE-PJ ; la section reste
  pertinente pour de futures actions AOE qui rouvriraient un différé). Seule différence PJ : les
  résultats par cible sont collectés dans un tableau et émis en **un** `COMBAT_ATTACK_PLAYER_RESULT
  { hit, roll, seuil, targets: [{ name, band, localisation, degautsBruts, degatsNets, severity }, …] }`,
  que `CombatModifiersWindow` affiche en liste (+ bouton « Fermer »). `combat_action_targets.outcome`
  écrit inline dans la boucle (déjà le cas côté PNJ).
- **Ce que ça ne touche pas** : `confirmDamage`, `armAwaitingDamage`, le pipeline différé, la FSM
  `AWAITING_DAMAGE`, `resolveAssaultHitPj`. Zéro code chaud partagé.

Les 4 helpers d'aggradation envisagés dans l'analyse à charge du 2026-09-03
(`buildAssaultDamagePending`, `armAwaitingDamageBatch`, `writeAoeTargetOutcome`, `applySpreadDamageDice`)
**tombent** : plus de 4ᵉ chemin de résolution, donc plus de duplication à créer. Le collapse du dispatch
par type de cible (3 exemplaires aujourd'hui : boucle AOE-immédiate, `resolveAssaultHitPnj*`,
`confirmDamage`) reste une dette distincte — `ROADMAP.md` §5, pas cette tranche.
...
8. Orchestration couche 4 : fan-out PNJ immédiat / PJ différé en parallèle, y compris auto-ciblage du lanceur (§5.1/§5.5), Test de Chance simplifié en v1 (§5.2, définitif), reprise MJ via le chemin `forced=true` déjà existant (§5.4) — aucune nouvelle file de confirmation (§5.3). **Fait pour le fusil à pompe tireur PNJ.**
9. UI de ciblage : mode "placer un point" et mode "choisir une direction/cône" — vérifier le cas auto-ciblage (§5.5) ne produit pas une séquence déroutante côté client. **Fait (close, `117b18a`).**
10. **Tranche tireur PJ du fusil à pompe [2026-09-03]** — le « PJ différé en parallèle » de l'étape 8 est **écarté** (§5.1 révisé). Design : retirer l'early-return `character.type === 'pj'` de `resolveAoeAssaultAction` ; après `hitTargets` + jet Phase A + décrément munition (partagés), tireur PJ = **la même boucle de résolution immédiate que le PNJ**, avec en plus la collecte des résultats par cible et un `COMBAT_ATTACK_PLAYER_RESULT { hit, roll, seuil, targets: [...] }` agrégé ; `CombatModifiersWindow` affiche la liste par cible + bouton « Fermer » ; 0 cible → `{ hit: false }`. ≈ 15 lignes serveur + une branche de rendu client + une clé i18n. Ne touche aucun code différé partagé.
...
- Couche 4 : scénario mixte PJ+PNJ dans une même explosion — toutes les cibles résolues immédiatement
  côté serveur (§5.1 révisé : plus de prompt différé par cible), le tireur PJ reçoit un
  `COMBAT_ATTACK_PLAYER_RESULT` agrégé listant chaque cible ; **répétition réseau du `COMBAT_ACTION_CONFIRM`**
  → dégât appliqué une seule fois (garde FSM `AWAITING_DAMAGE`/statut timeline `resolved` + `outcome IS NULL`, §3).
- Tranche tireur PJ (§8 étape 10) : PJ tire au Klauss en zone, 2-3 cibles à paliers différents
  (courte/moyenne/longue) dont une exo **ou** un drone → un dégât correct par cible, liste affichée
  dans `CombatModifiersWindow`, `combat_action_targets` + `outcome` peuplés, combat se termine
  normalement. Cas 0 cible (tir dans un couloir vide) → bannière « manqué », pas de blocage.
...
| 10. Tranche tireur PJ du fusil à pompe | `socketCombatHelpers.js#resolveAoeAssaultAction` + `client/src/lib/combatResultLabels.js` (nouveau) + `client/src/components/{CombatResultPanels,CombatModifiersWindow}.jsx` + `client/src/locales/combat.json` | **Clos, validé en session réelle par Saar (2026-09-03).** Deux plans intermédiaires écartés après conception (rework de fenêtres inexistant ; puis N `armAwaitingDamage` FIFO incompatible avec le hook client 1-cible — §5.1). **Codé** : (a) early-return `character.type === 'pj'` retiré ; `const isPnjResult = character.type !== 'pj'` paramètre `isPnj` dans les 4 émissions `COMBAT_ATTACK_RESULT` (préserve exo/drone tireur) ; (b) tireur PJ = **même boucle de résolution immédiate que le PNJ** + accumulation `playerTargetResults` ; (c) **un** `COMBAT_ATTACK_PLAYER_RESULT { hit, roll, seuil, targets: [{ name, band, localisation, degautsBruts, degatsNets, severity }] }` (to socket ; 0 cible → `{ hit: false, aoeNoTargets: true }`) ; (d) `CombatModifiersWindow` affiche la liste + bouton « Fermer » (`LOC`/`SEVERITY` extraits dans `combatResultLabels.js` — `react-refresh/only-export-components` interdit l'export depuis un `.jsx`) ; (e) clés `modifiers.attackBanner.noTargets` + `modifiers.aoeResult.*`. `{ suspend: false }`. **N'a touché ni** `confirmDamage`/`armAwaitingDamage`/pipeline différé/FSM/`resolveAssaultHitPj`. Vérifs préalables : précheck `assault` tolère l'absence de `target_token_id` ; bannière `attackResult` tolère `cibleTokenId: null` ; migration 317 sans contrainte unique mais ré-entrée bloquée FSM+timeline. | `node --check` serveur, `node --test 'shared/**'` 490/490, `socketCombatHelpers.test.mjs` 5/5, `eslint` (hunks propres), `npm run build`, `git diff --check` OK. Pas de test auto DB sur le chemin serveur. Session réelle Saar : OK |

**État de test au 2026-09-03** : chaînes ANNONCE (6b) **et** RÉSOLUTION (couches 2-4, étape 8)
confirmées en session réelle par Saar contre PostgreSQL — combat complet au Klauss en zone, tireur
PNJ, 3 cibles touchées, dégâts transmis, combat terminé normalement (`117b18a`). Un scénario plus
poussé reste souhaitable (cibles à paliers différents, une hors LOS, dégâts corrects par palier) mais
le fusil à pompe tireur PNJ est fonctionnellement clos. **Restent chacun un blocage explicite, pas un
oubli** : tireur PJ (contenu — ≈ 1 resolver + 1 spot, voir bannière révisée), lance-flammes (branche
resolveur à écrire, table de dégât RAW dispo — `REGLES_ARMES_SPECIALES.md:53-66`), grenades (catalogue
§6c + `intendedOrigin` + 2 pages RAW manquantes), tir de suppression (Test de Chance absent du schéma
+ zone persistante inter-tours — plus lourd, attend le chantier Chance).
