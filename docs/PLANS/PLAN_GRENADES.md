...
| **3c/2** | **Client — mode « viser un point »** (mirror des Segments 2a/2b) : ~13 fichiers. `aoePreviewShape.js` variante cercle (`buildCircleSpan` + `projectCircleRing`) ; `combatAoeTargetMode` gagne une variante `point` (le clic sol donne le POINT, pas une direction — `useSceneCursor`/`Canvas3D`) ; `aoeIntendedOrigin` threadé dans les 3 déclarations (`useExoDeclare`/`useDroneDeclare`/reducer humanoïde) + `build{Attack,Drone,Exo}MapActions` ; libellé « Viser un point » quand `getAoeProfile(w)?.shape === 'circle'` ; borne de portée de lancer `[INCONNU — v1 : généreux, affiné plus tard]`. **UI — validation Saar.** | client UI | golden-master `buildDeclarePayload` + lint + build + session |
