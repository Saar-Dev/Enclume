...
| **3c/2b** | **Client — mode « viser un point »** (mirror Segments 2a/2b) : ~10 fichiers d'**interaction 3D**. `combatAoeTargetMode` gagne un discriminant `point` vs `direction` — le survol/clic sol donne un POINT `{x,y,z}` (pas un cap) : `Canvas3D.jsx` (`useFrame` hover, handler de clic, branche de rendu `shape==='circle'` → `projectCircleFan`), `useSceneCursor.js`, `CombatOverlay.jsx` (relais). `aoeIntendedOrigin` threadé dans `useExoDeclare`/`useDroneDeclare`/reducer humanoïde + `build{Attack,Drone,Exo}MapActions` (`aoe: { intendedOrigin }`). Libellé « Viser un point » si `getAoeProfile(w)?.shape === 'circle'`. Rayon d'aperçu depuis `aoe_profile.radiusM`. **Portée de lancer : AUCUNE limite client v1** — RAW muet (Action simple, gate = Test de Coordination). Écart RAW (silence → pas de plafond) → `JOURNAL8.md` en 3g. **Chantier à part entière — session Saar dans la boucle.** | client UI 3D | golden-master `buildDeclarePayload` + lint + build + session |
| **migration** | `aoe_profile` `{shape:'circle', mechanic:'grenade_frag'}` sur la ligne fragmentation. **Après 3c/2** — sinon `isAoeWeapon(grenade)` rend la grenade éligible à une visée cassée. | migration | — |
