...
| **3c/2b-4** | **Fenêtres de déclaration arment le mode point + stockent `aoeIntendedOrigin`** (mirror Segments 2a/2b, per‑plateforme) : `useExoDeclare`+`CombatExoActionWindow` · `useDroneDeclare`+`CombatActionWindow`/`CombatGmDeclareWindow` · reducer humanoïde+`AssaultRangedPanel`. Exclusivité avec la cible, reset au changement de slot, libellé « Viser un point » (`assaultPanel.aimAoePointButton`). **UI par plateforme — sessions Saar.** | client UI | golden-master + lint + build + sessions |
