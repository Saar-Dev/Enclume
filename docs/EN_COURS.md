...
| **EXOARM-COMBATFILE** | Chantier Exo-armures (`docs/PLANS/PLAN_EXOARMURE.md`) Lot 1 codé (non testé navigateur), Lot 2 bloqué. Refactor `docs/PLANS/PLAN_COMBATANT_CONTEXT.md` Lots A-F ✅ clos (détail complet `docs/JOURNAL8.md` Session 2026-08-13) — `combatantContextService.js` point de couture unique, 7 sites migrés, imports morts et doc (`docs/SYSTEME/COMBAT.md`) nettoyés. Reste : (1) validation jeu réel des 2 branches "terrain instable défenseur" (`resolveMeleeDefensePnj`/`confirmMeleeDefense`) — nécessite le choix explicite du modificateur situationnel `cac_terrain_instable` à la Déclaration, jamais sélectionné dans les combats testés à ce jour ; (2) Lot G (dispatcher `resolveCombatantTestContext` + `resolveExoTestContext`) — seul lot qui débloque réellement le Lot 2 Exo-armures, dépend aussi de `computeExoStats` (calcul externe, `PLAN_EXOARMURE.md`, pas encore construit) | En cours — Lot G ensuite |
