...
**Abandonné (Saar, 2026-08-23)** : `docs/Old/PLAN_FICHE_HORSLIGNE.md` (fiche personnage hors-ligne,
PWA) — décision produit, remplacé par un besoin d'export Google Sheets (chantier distinct, pas encore
cadré, voir `docs/ROADMAP.md` § Chantiers futurs). Les 5 lots (A/B0/B/C/D) restent commités
(`vite-plugin-pwa`, routes hors session, cache Workbox, file `workbox-background-sync`, vue
d'impression) — code non retiré, seulement déprioritisé ; la vue d'impression (Lot D) avait déjà été
testée fonctionnelle en navigateur par Saar le 2026-08-16, le reste (mode hors-ligne effectif, rejeu
réseau) n'a jamais été validé et ne le sera pas dans ce cadre. Plan déplacé vers `docs/Old/`.
...
- P59-P62 — statuts de token et cadavre : rôle de statut lu dans `shared/tokenStatusRegistry.js` (jamais un littéral), « peut-il agir ? » = `getDeclarationBlockedTokens`, cadavre = `isCharacterDead` (reste une cible qui prend des blessures ; ni Chance ni Choc ni état de corps vivant), écriture automatique d'étourdi = `applyStunWithDuration` — `docs/SYSTEME/CONVENTIONS.md` §19 et `docs/SYSTEME/STATUTS_TOKEN.md`
- P63-P65 — blessures et 6ᵉ ligne : statut posé par une cause = marque de provenance + « insérer si absent » (jamais `applyModStatus`), gravité avec échéance de guérison = entrée `WOUND_INFECTION` obligatoire (`getWoundHealing`, `improvedSeverity`), règle des cases `isWoundLineFull` (ligne pleine = toutes les cases cochées, la blessure suivante convertit ; aucune exception Mortelle) — `docs/SYSTEME/CONVENTIONS.md` §19 et `docs/SYSTEME/BLESSURES.md`
