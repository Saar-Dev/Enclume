...
| **EXOARM-COMBATFILE** | Chantier Exo-armures (`docs/PLANS/PLAN_EXOARMURE.md`) Lot 1 codé (non testé navigateur). `docs/Old/PLAN_COMBATANT_CONTEXT.md` (Lots A-G) ✅ intégralement clos (détail `docs/JOURNAL8.md` Sessions 2026-08-13/14/15). **Lot 2 intégralement codé (2026-08-18)** : plafond de Compétence par Manœuvre d'armure ✅ (commit `7247ebb`, 2026-08-15) ; 1 seule Attaque/Tour sans code nécessaire (règle avancée dont elle dépend jamais implémentée, `PLAN_EXOARMURE.md` §7.7) ; routage de la confirmation de défense pour un `type='exo'` ✅ (`resolveCombatantIdentity`, §7.7 — 25/25 tests verts contre PostgreSQL réel, `docs/JOURNAL8.md` session du même jour). Reste uniquement la validation en jeu réel (aucune exo-armure en base à ce jour) : plafond de Compétence, routage confirmation défense, et les 2 branches "terrain instable défenseur" déjà en attente (modificateur `cac_terrain_instable`, jamais sélectionné en combat testé à ce jour). Token du pilote pendant qu'il pilote (`MANUEL_EXOARMURE.md` §6.3) reste géré à la table par décision actée, aucune mécanique serveur prévue | En attente — validation jeu réel (nécessite une exo-armure en base) |
