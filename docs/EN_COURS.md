...
| ~~**WIZ26**~~ | ~~`docs/BUG WIZARD.md` #28 : Step5 Avantages/Désavantages, page chargée, Saar demande un regroupement (idée initiale : deux colonnes, puis validation de l'option "grouper par famille"). Vérification en base (`ref_advantages`, migration 92) : `family` n'est PAS une taxonomie complète — champ étroit servant uniquement `family_limit` (variantes mutuellement exclusives : Phobie, Allergie, Déséquilibre mental, Sens développé...), la majorité des items n'en ont pas~~ | ⚠️ clos partiel Session (Saar, 2026-08-12) — `Step5Advantages.jsx` : `groupByFamily()` sépare `ungrouped` (grille plate inchangée, majorité des items) de `families` (bloc dédié par famille, en-tête + compteur "X/Y sélectionné(s)", clé i18n `step5.family_limit_counter`). Purement informatif : pas de blocage client à la limite atteinte (déjà appliqué serveur, `advantageConstraints.js`) — hors scope de ce bug. ESLint clean, build client OK — codé, scénario réel navigateur non testé |
