...
| ~~**WIZ6**~~ | ~~Finalisation : compétences remises à zéro — écho `WIZARD_STATE_SYNC` auto-diffusé à l'émetteur avec `skillAllocations: {}` (getStep4State), réinjecté par `openPeek`/`handleTerminate`~~ | ⚠️ clos partiel Session (Saar, 2026-08-11) — persistance brute `char_pc_ledger.skill_allocations`/`autodidacte_allocations` (migration 236), détail `docs/JOURNAL8.md` — codé, scénario réel navigateur non testé |
...
| ~~**WIZ20**~~ | ~~`docs/BUG WIZARD.md` #24 / `BUGIDENTIFIE.md` #8 : boutons +/- non harmonisés sur le Wizard — 3 classes CSS pour le même pattern stepper (`wiz1-spin-btn` Step1, `wiz4-stepbtn` Step4 années de carrière, `wiz4-sbtn` Step4 grilles de points), dont 2 quasi identiques mais dérivées à des valeurs différentes~~ | ⚠️ clos partiel Session (Saar, 2026-08-11) — `wiz1-spin-btn`/`wiz4-stepbtn` fusionnées en `.wiz-spin-btn` (convention `wiz-*` transverse), `wiz4-sbtn` conservé distinct pour les grilles denses (décision confirmée par Saar). Glyphe `+` ASCII uniformisé en `＋`. ESLint clean, build client OK — codé, scénario réel navigateur non testé |
| ~~**WIZ21**~~ | ~~Un MJ rejoignant une fiche déjà avancée sur Step4 (Profession) atterrissait systématiquement sur Récap au lieu de la sous-étape réellement affichée chez le joueur — gap distinct de WIZ15 (celui-ci gérait le remontage pendant l'observation, pas le calcul initial au premier montage, resté `initialData ? SUMMARY : AGE` sans distinction joueur/MJ)~~ | ⚠️ clos partiel Session (Saar, 2026-08-11) — `subStep` diffusé en direct via le canal `onLiveChange` déjà existant (ajouté uniquement au niveau de l'appel, jamais dans `buildPayload` lui-même — pas de champ UI dans la soumission serveur). Wrapper externe (`Step4Experience`) suit ce signal côté MJ uniquement (`gmSyncKey`), jamais côté joueur. Pattern "adjusting state during render" (déjà utilisé `SidebarChatTab.jsx`) plutôt que `setState` en effet (`react-hooks/set-state-in-effect`). ESLint clean, build client OK — codé, scénario réel navigateur (MJ rejoignant en cours de partie) non testé |
...
| ~~**WIZ23**~~ | ~~Fonctionnalité demandée par Saar (retours positifs sur les illustrations de métier, Step4) : cliquer sur l'illustration pour l'agrandir/rétrécir~~ | ⚠️ clos partiel Session (Saar, 2026-08-11) — `CareersAllocator.jsx` : overlay plein écran au clic (`object-fit: contain`, image entière visible — la vignette est recadrée en `cover`), clic n'importe où dans l'overlay pour refermer, même patron que `SidebarHelpModal.jsx`. Scope limité à cette illustration : les 3 autres emplacements du Wizard (Step0/Step2/Step3, `wiz-card`/`wiz2-card`) utilisent déjà l'image comme cible de clic pour la sélection — même geste en conflit, pas traité ici. ESLint clean, build client OK — codé, scénario réel navigateur non testé |
| ~~**WIZ24**~~ | ~~`docs/BUG WIZARD.md` #25 : icône ⚠ (`wiz4-restr`, restriction géographique d'une carrière, Step4 rail gauche) trop petite — héritait de `.wiz4-railmeta` (10px, taille du texte environnant salaire/rang)~~ | ⚠️ clos partiel Session (Saar, 2026-08-11) — `font-size: 13px`/`line-height: 1` posés directement sur `.wiz4-restr`, seul usage (`CareersAllocator.jsx`). CSS pur, build client OK — codé, scénario réel navigateur non testé |
| ~~**WIZ25**~~ | ~~Suite à WIZ24 (bug #25) : Saar demande que le tooltip de restriction géographique apparaisse au survol de l'ensemble titre + sous-titre du métier (Step4 rail), pas seulement l'icône ⚠ elle-même, et après un délai (pas instantané)~~ | ⚠️ clos partiel Session (Saar, 2026-08-11) — `title` natif remplacé par un tooltip custom (`CareersAllocator.jsx`, délai 700ms, `setTimeout`/`clearTimeout`), déclenché sur le bloc titre+sous-titre. Classe `wiz1-tooltip` (Step1Attributes.jsx) renommée `wiz-tooltip` (transverse, même patron que `wiz-spin-btn` WIZ20) et réutilisée telle quelle. ESLint clean, build client OK — codé, scénario réel navigateur non testé (délai à valider en usage réel) |
