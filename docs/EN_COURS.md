...
| **WIZ13** | Signalement beta-testeur (pas de pas-à-pas, navigation intensive entre étapes/brouillons pendant les tests) : `select * from char_sheet where id = $1 ... invalid input syntax for type uuid: "null"`. Cause confirmée du crash : `resolveSheetAccess` (`creationService.js`, garde REST + WebSocket) interrogeait la base avec un `sheetId` non validé — corrigé (garde-fou format + `AppError(404)` propre + log `[DBG-WIZNULL]`). **Cause côté client NON trouvée** : tous les chemins tracés (nouvelle création, reprise via URL, reset MJ `resetCreation`, liste de brouillons `CharacterPoolPage.jsx`) valident `sheetId` avant de l'utiliser ou ne peuvent structurellement pas être `null` (clé primaire NOT NULL). Reste ouvert tant que le `[DBG-WIZNULL]` n'aura pas capturé une occurrence réelle | Moyenne — crash serveur neutralisé (404 propre au lieu d'un 500 brut), cause client à identifier via les logs si ça se reproduit |
