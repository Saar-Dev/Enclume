...
| **EXOARM-COMBATFILE** | Chantier Exo-armures (`docs/PLANS/PLAN_EXOARMURE.md`) Lot 1 codé (non testé navigateur). `docs/Old/PLAN_COMBATANT_CONTEXT.md` (Lots A-G) ✅ intégralement clos. **Lot 2 intégralement codé (2026-08-18)** : plafond de Compétence, routage confirmation défense, 1 seule Attaque/Tour (sans code nécessaire). **Lot 2bis (Armure à terre) codé (2026-08-19)** : Test de Manœuvre d'armure pour se relever (`resolveExoStandUpAction`, migration `249`, fenêtre dédiée `CombatExoActionWindow.jsx` joueur+MJ), plus un trou de permission trouvé et corrigé au passage (`isExoActorAuthorized` — un pilote ≠ propriétaire ne pouvait déclarer aucune action, pas seulement se relever). Détail complet `docs/JOURNAL8.md`. Reste uniquement la validation en jeu réel (aucune exo-armure en base à ce jour) : plafond de Compétence, routage confirmation défense, Armure à terre, et les 2 branches "terrain instable défenseur" déjà en attente. Token du pilote pendant qu'il pilote (`MANUEL_EXOARMURE.md` §6.3) reste géré à la table par décision actée, aucune mécanique serveur prévue | En attente — validation jeu réel (nécessite une exo-armure en base) |
