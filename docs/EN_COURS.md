...
| CH1 | Historique chat perdu au F5 (rechargement page) — Phases 1 et 2 (`docs/PLANS/PLAN_CHAT.md`) ✅ closes 2026-08-05 : module `server/src/chat/` (33 tests, vérifiés contre PostgreSQL réel), double-écriture non bloquante dans le handler `CHAT_MESSAGE` existant derrière `CHAT_PERSISTENCE_ENABLED` (défaut false). Reste Phase 3 (bascule client lecture + pagination) et 4 (nettoyage) | Chantier dédié — flag encore désactivé par défaut, non testé en navigateur avec persistance active |
