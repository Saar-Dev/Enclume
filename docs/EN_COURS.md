...
| **EXOARM-COMBATFILE** | Chantier Exo-armures (`docs/PLANS/PLAN_EXOARMURE.md`) Lot 1 codé (non testé navigateur), Lot 2 resserré : mouvement (VIT) codé, plafond de Compétence (Manœuvre d'armure) et "1 seule Attaque/Tour" bloqués — refactor `docs/PLANS/PLAN_COMBATANT_CONTEXT.md` en cours (Lots A-G) : Lot A (`combatantContextService.js`, 9 tests) ✅ + Lot B (site attaquant CaC `resolveMeleeAction` branché, fetch inline retiré) ✅ — validé par Scientist en jeu réel (3 combats CaC, dont un à modificateurs cumulés, 0 écart) avant bascule. Reste sites #1/#3 (cible/défenseur CaC, Lot C), #4/#5/#7 (cibles Tir/Drone, Lot E), #6 (tireur, Lot D), nettoyage (Lot F), dispatcher + `resolveExoTestContext` (Lot G, seul lot qui débloque réellement le Lot 2 Exo-armures) | En cours — Lot C ensuite |
