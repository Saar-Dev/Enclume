...
| ~~**HORLOGE-OVERFLOW1**~~ | ~~`deltaMinutes` sans borne haute face au type `integer` Postgres — erreur SQL brute renvoyée au client~~ | ✅ Session (Saar, 2026-08-01) — `assertWithinPgInteger` sur `displayedAfter`/`resolvedAfter`, `gameTimeService.js`, 3 tests DB réels ajoutés, non testable en jeu (aucun flux UI ne peut produire ce delta) |
| ~~**COMBAT-LOS-PRECHECK-DIVERGENCE**~~ | ~~Precheck Tir (`COMBAT_ACTION_PRECHECK`) ne vérifiait ni portée ni LOS, contrairement au precheck CaC~~ | ✅ Session (Saar, 2026-08-01) — `checkLOSForPrecheck` réintégré, `socketCombatResolution.js`, jamais vérifié en situation réelle de Tir bloqué par LOS, détail `docs/BUGIDENTIFIE.md` |
