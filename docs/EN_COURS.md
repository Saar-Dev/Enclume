...
| ~~**MELEE-INHAND**~~ | ~~Arme principale CaC jamais vérifiée "en main" à la résolution~~ | ⚠️ clos partiel — Session (Saar, 2026-08-05) : refactor `getOwnedHandWeapon` (inventoryService.js), autorité unique CaC+Tir/principale+secondaire/Déclaration+Résolution, 192/192 tests serveur verts, scénario réel navigateur non testé |
| ~~**ASSAULT-INHAND-RESOLUTION**~~ | ~~Tir : ownership/en-main jamais revérifiés à la résolution~~ | ⚠️ clos partiel — Session (Saar, 2026-08-05) : `fetchAssaultWeaponAndMods` migré vers `getOwnedHandWeapon`, même commit que MELEE-INHAND, scénario réel navigateur non testé |
...
| **INVUX-679** | `PLAN_INVENTORY_UX.md` Étapes 6-8 codées + Étape 9 partielle (2026-08-05) — filtres/pagination catalogue GM, confirmation suppression, séparation Coffre, bouton "Prendre dans le Sac"/"Ranger dans le Coffre", libellés de slot traduits. Au passage : retrait du `<select>` container (demande directe Saar) et correctif `InteractiveAwarePointerSensor` (bug clic-select → drag). Non testé en navigateur | ⚠️ clos partiel — validation navigateur par Saar |
