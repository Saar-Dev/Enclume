...
| **EXOEQ-FUSION1** | Fusion candidate `ref_exo_equipment` → `ref_equipment` (famille unique "Exo-armure") envisagée pendant la transcription des loadouts (§13.4.4), non faite — audit d'impact complet dans `PLAN_EXOARMURE.md`. Obstacle bloquant trouvé : `ref_equipment.tech_level` est `integer` (comparé numériquement dans `tradeService.js`), `ref_exo_equipment.tech_level` est `text` ("III-IV", "Selon NT") — pas de conversion triviale. Fusionner résoudrait aussi le ticket marchand ouvert séparément (`c9915238-ff6c-4073-8d13-75488f250d9b`, `/admin/tickets`) mais de façon non maîtrisée (tout marchand exposerait soudain tout le catalogue exo) sans décision produit explicite sur le filtrage. Rendrait aussi obsolète l'exclusive arc de la migration 260 (faite/testée le même jour) | Basse — refonte future, touche `ref_equipment` dans son ensemble (678 lignes, tout le système marchand), pas seulement l'exo |
