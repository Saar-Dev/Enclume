...
| **GAUGES-MATERIEL** | Jauges de Matériel & liste d'attente Wizard (`docs/PLANS/PLAN_WIZARD_MATERIEL_GAUGES.md`) — droits joueur sur `InventoryPanel` Step6, validation MJ item par item, jauges `char_gauges` (ressource persistante Wizard + fiche permanente). Codé de bout en bout (migration 242, routes, store, `GaugesPanel.jsx`) — **committé, non testé en navigateur** | ⚠️ clos partiel — validation navigateur par Saar avant clôture |
