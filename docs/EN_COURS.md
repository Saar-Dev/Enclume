...
| **EXOARM-COMBATFILE** | Chantier Exo-armures (`docs/PLANS/PLAN_EXOARMURE.md`) Lot 1 codé (non testé navigateur), Lot 2 bloqué. `docs/Old/PLAN_COMBATANT_CONTEXT.md` (Lots A-G) ✅ intégralement clos (détail `docs/JOURNAL8.md` Sessions 2026-08-13/14/15) — `combatantContextService.js` point de couture unique (dispatcher `resolveCombatantTestContext` + branche exo assemblée), tous les sites `socketCombatHelpers.js` rebranchés, testé (18 tests verts contre PostgreSQL réel, dont la branche exo : pilote+template, paliers d'Intégrité dégradés, sans pilote, sans template). Un pilote d'exo-armure peut désormais Tester (Seuil/malus/ModDom dérivés du pilote + EXF) sans crash ni blocage silencieux. Décision actée (2026-08-15) : le token du pilote pendant qu'il pilote (`MANUEL_EXOARMURE.md` §6.3) reste géré à la table, aucune mécanique serveur — à reporter dans `PLAN_EXOARMURE.md` quand ce document reprendra. Reste : (1) validation jeu réel des 2 branches "terrain instable défenseur" (nécessite le choix explicite du modificateur situationnel `cac_terrain_instable` à la Déclaration, jamais sélectionné dans les combats testés à ce jour) ; (2) `PLAN_EXOARMURE.md` Lot 2 (plafond de Compétence par Manœuvre d'armure, 1 seule Attaque/Tour, routage de la confirmation de défense pour un `type='exo'`) — peut démarrer, plus aucune dépendance bloquante côté `PLAN_COMBATANT_CONTEXT.md` | En attente — reprise `PLAN_EXOARMURE.md` Lot 2 |
