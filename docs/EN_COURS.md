...
| **EXOARM-COMBATFILE** | Chantier Exo-armures (`docs/PLANS/PLAN_EXOARMURE.md`) Lot 1 codé (non testé navigateur), Lot 2 resserré : mouvement (VIT) codé, plafond de Compétence (Manœuvre d'armure) et "1 seule Attaque/Tour" bloqués — refactor `docs/PLANS/PLAN_COMBATANT_CONTEXT.md` en cours (Lots A-G) : Lot A (`combatantContextService.js`, 10 tests) ✅ + Lot B (attaquant CaC) ✅ validé Scientist en jeu réel + Lot C (défenseur CaC, périmètre élargi à 3 sites, site #1 retiré — aucune duplication) ✅ codé, bloc principal validable par tout combat CaC normal, branches terrain instable non testées en jeu (déclenchement rare) + Lot D (tireur `resolveAssaultAction`) ✅ codé — palier complet forcé via `skillId=''` quand `ref_equipment_skill_assoc` ne trouve pas l'arme (jamais `null`, qui routerait à tort vers le palier NA seul et perdrait effectiveMalus/for_na pour un tireur actif — comportement désormais verrouillé par test unitaire dédié), `node --check`/10 tests verts — **confirmé en jeu réel** (2 tirs réels, PNJ Seuil 16 raté + PJ Seuil 21 touché, aucune erreur). Lot C bloc défenseur principal reconfirmé par 2 défenses CaC réelles supplémentaires. `calcSkillTotal`/`calcAttributeNA`/`calcActiveMalus`/`getMutationEffects` n'ont plus aucun appelant dans `socketCombatHelpers.js` (constaté, pas encore nettoyé — réservé au Lot F). Lot E investigué et clos : les sites #4/#5/#7 (cibles Tir/Drone) sont déjà unifiés via `damageService.fetchCibleNA`, extrait le 2026-07-30 dans `PLAN_FATIGUE_DOMMAGES.md` §9 — antérieur à ce plan, jamais recroisé par son audit initial. Rien à faire côté `socketCombatHelpers.js`. En revanche le palier "NA seul" de `combatantContextService.js` (Lot A) dupliquait ce même calcul sans le savoir (jamais exercé par un site réel avant le Lot E) — corrigé : délègue désormais à `fetchCibleNA`, 9 tests toujours verts, résultat numérique identique. Reste nettoyage (Lot F, y compris grep de contrôle "aucun site ne fait plus de fetch char_sheet direct"), dispatcher + `resolveExoTestContext` (Lot G, seul lot qui débloque réellement le Lot 2 Exo-armures) | ⚠️ clos partiel — reste uniquement les 2 branches "terrain instable défenseur" du Lot C (`resolveMeleeDefensePnj`/`confirmMeleeDefense`) : nécessitent le choix explicite du modificateur situationnel `cac_terrain_instable` à la Déclaration (pas de détection automatique côté défenseur, contrairement à l'attaquant) — 2 combats CaC testés depuis la clôture du Lot C, aucun ne l'a sélectionné |
