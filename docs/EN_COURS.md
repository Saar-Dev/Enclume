...
| **EXOARM-COMBATFILE** | Chantier Exo-armures (`docs/PLANS/PLAN_EXOARMURE.md`) Lot 1 codé (non testé navigateur), Lot 2 resserré : mouvement (VIT) codé, plafond de Compétence (Manœuvre d'armure) et "1 seule Attaque/Tour" bloqués — refactor `docs/PLANS/PLAN_COMBATANT_CONTEXT.md` en cours (Lots A-G) : Lot A (`combatantContextService.js`, 9 tests) ✅ + Lot B (attaquant CaC) ✅ validé Scientist en jeu réel + Lot C (défenseur CaC — périmètre élargi en analyse à charge : 3 sites branchés, pas 1 seul comme prévu au plan initial — `resolveMeleeAction` bloc défenseur, `resolveMeleeDefensePnj` et `confirmMeleeDefense` terrain instable ; site #1 `isTargetDefenseless` retiré du lot, aucune duplication n'y existait) ✅ codé, `node --check`/tests unitaires verts — bloc défenseur principal validable par tout combat CaC normal (PJ et PNJ défenseurs), branches terrain instable non testées en jeu (déclenchement rare, nécessite case spéciale ou modificateur situationnel explicite). Reste #4/#5/#7 (cibles Tir/Drone, Lot E), #6 (tireur, Lot D), nettoyage (Lot F), dispatcher + `resolveExoTestContext` (Lot G, seul lot qui débloque réellement le Lot 2 Exo-armures) | ⚠️ clos partiel — validation jeu réel terrain instable en attente |
