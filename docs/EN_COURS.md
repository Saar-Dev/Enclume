...
| ~~**WIZ27**~~ | ~~`docs/BUG WIZARD.md` #18 (+ #27 dépendant) : aucun texte de tutoriel en haut de chaque étape du Wizard — textes déjà rédigés par Saar, composant `StepTutorial` restait à créer~~ | ⚠️ clos partiel Session (Saar, 2026-08-12) — `StepTutorial.jsx` créé, un seul point d'intégration dans `WizardCreation.jsx` (pas de duplication dans les 7 composants d'étape comme le suggérait le doc). Steps 0/1/2/3/4/5/7 couverts. Step 6 (Matériel et biens, step ajouté après la rédaction du bug #18, absent de son périmètre d'origine) sans texte — composant tolère l'absence de clé (rendu `null`), à compléter quand Saar aura rédigé le texte. `en.json` non touché (anglais gelé). ESLint clean, build client OK — codé, scénario réel navigateur non testé |
