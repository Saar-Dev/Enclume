VOCABULARY.md — Contrat sémantique officiel d'Enclume
    ...
    Complément 2026-09-25 (sans changement de version) : ajout « Permuter (objet en main) » (chantier « Permuter l'arme en combat »,
...
Mort (statut)	Statut de token `dead` : le personnage est mort. Posé et retiré par le MJ seul (jamais par un joueur, quelle que soit l'option de campagne) ; le token est passé automatiquement (jamais de fenêtre d'action), sans défense, et devient un cadavre. Posé aussi automatiquement par la blessure « Mort » (6ᵉ ligne du compteur, en Tête/Corps), qui le retire avec elle (marque `data.source = 'wound'` : un `dead` posé à la main n'est jamais retiré par une blessure). `/heal` soigne tout, y compris un `dead` posé à la main. À ne pas confondre avec « Mort subite » (la gravité, voir ci-dessous).	token_statuses.status_code = 'dead', shared/tokenStatusRegistry.js (`isDeath`), docs/SYSTEME/STATUTS_TOKEN.md
...
Mort subite / Membre détruit (6ᵉ ligne)	Sixième ligne du compteur de blessures (seuil de Dommages 30, cible du débordement de la ligne Mortelle) : UNE seule gravité stockée, `mort_subite`, dont le libellé dépend de la localisation — « Mort » en Tête/Corps (le personnage meurt sur le coup : pose le statut `dead`, aucun Test de Choc, aucune guérison), « Membre détruit » sur un bras/une jambe (le personnage ne meurt pas : Tests interdits, Choc −10, jambe = déplacement impossible, guérison 3 semaines vers une Blessure critique). Une seule case, affichée comme un mot ; pose et retrait manuels réservés au MJ.	shared/woundConstants.js (`WOUND_SEVERITIES`, `isSuddenDeathLocation`, `isFatalWound`, `getWoundHealing`), docs/SYSTEME/BLESSURES.md
Débordement (ligne Mortelle)	Une Mortelle qui dépasse la capacité de sa ligne (2ᵉ Mortelle à la tête, 3ᵉ au corps) écrit la 6ᵉ ligne. Exception à la règle générale « la blessure qui remplirait la dernière case convertit » : sans elle, toute Mortelle à 1 case deviendrait aussitôt Mort (d'où l'importance des casques).	shared/woundConstants.js (`OVERFLOW_ONLY_SEVERITIES`, `isWoundLinePromoted`)
...
Permuter (objet en main)	Action de combat qui met en main un objet tenable du Sac ou de la Ceinture (bouton ⇄), en rangeant d'abord ce qui gêne, en une seule opération. Ne pas confondre avec « Rangée / Au clair » (état de l'arme du personnage, `state_weapon`, qui se change séparément) ni avec « Équiper » (geste libre de la fiche, sans coût de combat)	inventoryService.swapItemInHand ; shared/combatGrabItem.js ; docs/SYSTEME/COMBAT.md « Permuter l'arme en combat »
