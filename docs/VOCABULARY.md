...
| Marge de réussite / d'échec   | `mr` (signé, +réussite/-échec)    | `resolveTestOutcome` (`shared/polarisTestResolution.js`) | LdB p.203           | Marge de réussite = résultat du dé lu directement (**pas** Seuil−jet). Marge d'échec = jet−Seuil. Détermine le degré ("Bon", "Catastrophique"...) et le modificateur associé (`MR_TABLE`, ex-table DB `polaris_mr`). Ne pas confondre avec "Réussite/Échec critique" (déclenchées par le jet, pas la marge — voir ci-dessous). |
| Réussite critique / Échec critique / Catastrophe | — | `resolveTestOutcome` | LdB p.204-205 | Réussite critique = jet === Seuil final (jamais jet===1). Échec critique = jet===20 (retest, s'ajoute à la marge d'échec). Catastrophe = marge d'échec ≥15, **jamais automatique** (décision MJ, règle optionnelle) — distincte de l'Échec critique qui la rend seulement possible. Historique de correction : `docs/PLAN_TEST_CRITIQUE.md`. |
| Marge de réussite/échec        | `mr` (existant, `combatAttackRoll.js`) | Table RAW "Marges & modificateurs" — détermine le **degré** de réussite/échec (et la vraie règle de réussite/Catastrophe critique, pas `roll===1`/`roll===20`) | LdB | **Cadrage en cours, pas encore confirmé** — `docs/PLAN_TEST_CRITIQUE.md`. `roll===1`/`roll===20` (déjà en production dans plusieurs fichiers combat) est une simplification non-RAW à corriger, réglage de campagne prévu pour ne pas casser l'existant pendant la transition. Ne pas ajouter de nouveau site `roll===1`/`===20` tant que ce chantier n'est pas tranché. |
...
| Exo-armure                     | —                                 | `characters.type='exo'` (à venir), `exo_sheet` | LdB (`docs/REGLES/REGLEARMURE.md`, p.323-329) | Armure mécanisée pilotable — catégorie de `character` à part entière (comme `'pj'/'pnj'/'drone'`), **pas un drone** : le drone est autonome (programmes propres), l'exo-armure dépend structurellement d'un pilote humain (Initiative `min(Réaction, Manœuvre d'armure)`, plafond de Compétences, dégâts "Pilote" en cas d'incident). Lien pilote dynamique et simple : `exo_sheet.pilot_character_id` (champ éditable, pas de FSM embarquer/débarquer). Stats effectives (EXF/VIT/BLD) jamais stockées — recalculées depuis l'intégrité courante des 3 composants (Structure/Exosquelette/Générateur), patron confirmé par recherche externe (Foundry VTT/Lancer, "derived data"). Chantier en cours, cadrage complet : `docs/PLAN_EXOARMURE.md`. |
...
| Verrou (Wizard collaboratif) | Le MJ gèle une option précise (`(sheetId, step, optionKey)`) pendant que le joueur crée son personnage — le joueur ne peut plus la choisir ni la retirer, jusqu'à déverrouillage. Absence de ligne = option libre. Diffusé en temps réel par room dédiée `wizard:<sheetId>`, jamais la room de campagne. | `wizard_locks` (migration 201), `shared/wizardOptionKeys.js`, `docs/PLAN_WIZARDCOLLAB.md` |
| Pool de personnages | **Jamais "Brouillon" côté UI/texte joueur ou MJ** (jugé dénigrant, décision Saar) — "brouillon"/"draft" reste le terme de code interne (`char_sheet.creation_state`, `startCreation`, `GET /campaign/:campaignId/drafts`), jamais affiché. Désigne côté UI l'écran MJ listant les personnages en cours de création de la campagne (Lot A3, `docs/PLAN_WIZARDCOLLAB.md`), pour ouvrir celui d'un joueur ou en démarrer un nouveau. Même patron que Coffre/Vault (nom de code interne ≠ terme UI). | `docs/PLAN_WIZARDCOLLAB.md` §4.1 |
...
| Carte 2D | Battlemap rendue à plat (caméra orthographique plongeante) plutôt que par le moteur voxel 3D — même autorité spatiale que le 3D (`WorldSnapshot`, tokens en `world-feet`), seulement une salle unique triviale (sol activé, murs désactivés) et un rendu différent. Couvre aussi bien une illustration pure (aucun token placé) qu'une scène légère à tokens — pas deux concepts séparés. Discriminant : `battlemaps.render_mode` (`'2d'`/`'3d'`, défaut `'3d'`, rétrocompatible). **Ne pas confondre avec Spotlight** (bibliothèque de présentation ponctuelle en surimpression, ne change jamais le lieu — besoin distinct, `docs/PLAN_BATTLEMAP2D.md` §2). | `battlemaps.render_mode`, `client/src/components/Canvas2D.jsx`, `docs/PLAN_BATTLEMAP2D.md` |
| `token_style` | Apparence du token 2D d'un personnage — forme (`circle`/`hex`/`square`), cadrage du portrait dans la forme (`crop.offsetX/offsetY/zoom`), bordure (`border.color/width`) et emplacement réservé `overlay` (non exploité en v1). Portée **personnage** (comme `portrait_url`/`glb_url`), pas token — un seul style valable sur toutes les cartes. Couches de rendu séparées (forme/bordure/overlay jamais fusionnées en une image), pattern vérifié sur Discord (`avatar_decoration_data`) et Foundry VTT (Dynamic Token Ring) avant de le retenir — voir `docs/PLAN_BATTLEMAP2D.md` §10 (Lot 5). | `characters.token_style` (migration 215, jsonb nullable), `client/src/components/TokenPresentation.jsx` |
| Horloge de campagne | Compteur de temps de jeu écoulé, en minutes, ajusté manuellement par le MJ dans les deux sens (jamais un minuteur temps réel). **Calendrier propre à Enclume, aucun rapport avec un calendrier réel** — 12 mois fixes de 31 jours (année de jeu = 372 jours), aucune longueur variable, aucun bissextile, aucun nom de mois (recherche exhaustive du Livre de Base : aucune matière trouvée, `docs/PLAN_FATIGUE_DOMMAGES.md` §7). **Deux compteurs, jamais un seul** : `game_time_minutes` (affiché/narratif, librement déplaçable, peut descendre sous 0) et `game_time_resolved_minutes` (repère mécanique interne, strictement non-décroissant, **ne quitte jamais le serveur** — ni HTTP ni WS, consommé par le futur balayage d'échéances du Lot 2). Départ de campagne (année/mois/jour) configurable par le MJ dans Options de campagne, onglet Règle du jeu. | `campaigns.game_time_minutes`/`game_time_resolved_minutes` (migration 217, integer — pas bigint, `pg-types` désérialise `bigint` en string, voir Architecture retenue du plan), `server/src/lib/gameTimeService.js` (`adjustGameTime`), `shared/gameTime.js` (`projectGameTime`, constantes 31/12/372), `client/src/components/GameTimeWidget.jsx` (branché via le store `campaignStore` + `useSessionSocket.js`, P57 `docs/SYSTEME/REACT.md`), `docs/PLAN_FATIGUE_DOMMAGES.md` §7 (Lot 1) |
| Fatigue (Compteur de Fatigue) | État de personnage gradué (règle avancée optionnelle, `settings.fatigue_enabled`, défaut désactivé) — 6 paliers (Normal → À bout de force, malus -0/-3/-5/-7/-10/-10 à tous les Tests sauf le Test de Fatigue lui-même) × 3 cases (malus supplémentaire au **prochain** Test de Fatigue/Choc uniquement, Annexe LdB p.250). **Deux malus indépendants, jamais confondus** — même principe qu'`Horloge de campagne` ci-dessus (une propriété = une autorité). Test de Fatigue = jet interactif réel (CON/VOL/Endurance/moyenne + modificateur MJ), résolution **immédiate** (pas un consommateur du moteur d'échéances, Lot 2) : échec → palier+1 case 0 ; Catastrophe (`catastropheRisk`, marge ≤ -15 — **pas** `isCriticalFail`, deux concepts RAW distincts) → palier+1 case 1, appliqué automatiquement (seule exception du projet où `catastropheRisk` a un effet mécanique) ; réussite → case+1 ou -1 si marge ≥15 (« second souffle »). Palier 5 : le Test est remplacé par un Test de Choc, échec → statut `evanoui`, Catastrophe → `unconscious` — sans expiration si hors combat (`current_turn` ne progresse jamais hors combat), retrait manuel MJ. **Malus de Fatigue/blessure/encombrement unifiés** dans un seul registre déclaratif, branché aussi sur le système de macros joueur (corrige un bug préexistant où les macros ignoraient blessure/encombrement). Pas de badge token persistant pour le palier (décision Saar) — visible uniquement sur la fiche de personnage. | `char_sheet.fatigue_points` (migration 227, integer 0-17), `shared/fatigueConstants.js`, `server/src/lib/activeMalusRegistry.js` (`calcActiveMalus`), `server/src/lib/fatigueService.js` (`resolveFatigueTest`/`restFatigue`/`setFatiguePoints`), `client/src/character/CharacterSheet.jsx`, `docs/PLAN_FATIGUE_DOMMAGES.md` §10 (Lot 4) |
| Froid (tranche/exposition) | Danger environnemental déclaré/retiré manuellement par le MJ sur un personnage (badge `hypothermia`, catégorie "chronique" — comme les futurs Maladies/Poisons/Irradiations, par opposition aux dangers "dot" du Lot 3 scope Tour de combat). 3 tranches de base (Froid/Très froid/Glacial, cadence 2h/1h/30min du Test de résistance à la Fatigue) — **Froid extrême n'est pas une 4ᵉ tranche**, c'est Glacial répété avec un diviseur (`extremeSteps`) qui double par palier de -5°C supplémentaire (RAW p.244), plus l'humidité qui divise indépendamment par deux. Glacial et en dessous ajoutent des dégâts physiques réels (Bras/Jambes dès la 1re heure, Corps/Tête dès la 2ᵉ, escalade 1D10/2D10/3D10...). Scope **personnage** (`character_id`), jamais token — le badge token n'est qu'un reflet visuel sans donnée mécanique, la tranche réelle ne vit que dans les échéances du Lot 2. Consomme deux fondations : le Test de Fatigue existant (`performFatigueTest`, extrait de `resolveFatigueTest` pour être appelable depuis un handler d'échéance sans transaction propre) et `resolveTargetHit` (moteur de blessures), dont l'application réelle est différée après le commit du balayage d'horloge (`applyColdDamageHits`) — `resolveTargetHit`/`applyWound` émet son WS de façon synchrone et inconditionnelle, jamais appelable depuis un savepoint pas encore committé. | `game_echeances` (`cold_fatigue_check`/`cold_damage_tick`, Lot 2, aucune migration dédiée — `payload` jsonb générique suffit), `shared/coldExposureConstants.js` (`computeColdIntervalMinutes`), `server/src/lib/coldExposureService.js`, `client/src/components/TokenStatusPanel.jsx` (sous-formulaire sur le badge `hypothermia`, déjà réservé mais jamais câblé avant ce Lot), `client/src/components/EnvironmentalResultQueue.jsx` (file d'attente d'affichage hors combat, bénéficie aussi à Feu/Acide/Décompression), `docs/PLAN_FATIGUE_DOMMAGES.md` §11 (Lot 5) |
...
| Carte 2D | Spotlight | Une carte (2D ou 3D) montre le lieu où le groupe se trouve ; le Spotlight montre ponctuellement quelque chose (personnage, document, indice) en surimpression sans changer de lieu. Deux besoins distincts identifiés dans la même session de cadrage (2026-07-25) — Spotlight reste hors périmètre de `docs/PLAN_BATTLEMAP2D.md`, plan séparé à écrire. |
