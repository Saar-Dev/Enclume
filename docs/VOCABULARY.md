...
Mort (statut)	Statut de token `dead` : le personnage est mort. Posé et retiré par le MJ seul (jamais par un joueur, quelle que soit l'option de campagne) ; il bloque toute action et rend sans défense. À ne pas confondre avec « Mort subite » (6ᵉ ligne du compteur de blessures, chantier PLAN_BLESSURE_SIXIEME_LIGNE) qui le posera automatiquement.	token_statuses.status_code = 'dead', shared/tokenStatusRegistry.js
