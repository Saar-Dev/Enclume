...
### P-SRV-9 — DNS système cassé (résolution npm/GitHub en échec, `EAI_AGAIN`)
Symptôme observé (2026-08-08) : `npm install` échoue avec `EAI_AGAIN getaddrinfo registry.npmjs.org`, `getent hosts <domaine>` ne renvoie rien pour **tous** les domaines (pas spécifique à npm).

Cause racine : le réseau de cet hôte n'est **pas** géré par `dhcpcd` malgré `dhcpcd-base` installé et `/etc/dhcpcd.conf` présent — il n'existe aucune unité systemd `dhcpcd.service` (confirmé : `systemctl start dhcpcd` → `Unit dhcpcd.service not found`). Le réseau réel est piloté par `networking.service` (ifupdown) + `wpa_supplicant`. `/etc/network/interfaces` déclare `wlxd8ec5e060d2c` en **`static`** avec `dns-nameservers 8.8.8.8 8.8.4.4` — mais cette directive ifupdown n'est appliquée à `/etc/resolv.conf` que par le paquet `resolvconf`, **absent** de cet hôte (`dpkg -l | grep resolvconf` → rien). Résultat : la déclaration DNS existe mais n'a jamais été appliquée ; `/etc/resolv.conf` ne contenait que le résidu placeholder d'un ancien fonctionnement en DHCP (`# Generated by dhcpcd`, sans ligne `nameserver`).

L'IP elle-même reste fonctionnelle (`ping 8.8.8.8` OK) car statique — seul le DNS est mort, ce qui explique pourquoi les symptômes touchent uniquement les commandes réseau par nom d'hôte (npm, git fetch sur domaine) et pas le routage IP.

**Fix appliqué** (durable — rien d'autre sur cet hôte ne gère `/etc/resolv.conf`, donc pas de risque d'écrasement) :
```bash
...
EOF
```
Vérifier : `getent hosts registry.npmjs.org`. Pas de bounce d'interface ni de service réseau redémarré → aucun risque pour une session SSH en cours.
...
# Si nouvelles dépendances npm : npm install À LA RACINE, DANS client/ ET DANS server/
# (les 3 ont des node_modules indépendants, pas de workspaces npm — un `npm install`
# dans un seul dossier ne couvre pas les autres ; vécu 2026-08-08 : @dnd-kit/core manquant
# côté client ET lru-cache manquant côté server après le même pull)
npm install
cd client && npm install && cd ..
cd server && npm install && cd ..
sudo systemctl restart enclume-server enclume-client
# Si ça ne redémarre rien de visible : vérifier d'abord que les unités existent et tournent
# (`systemctl status enclume-server enclume-client`) avant de lancer `npm run dev` à la main —
# le 2026-08-08 le service enclume-server n'apparaissait pas dans `ps aux` sans qu'on ait
# confirmé si l'unité systemd était absente, désactivée ou juste arrêtée ; à trancher la
# prochaine fois plutôt que de redémarrer en foreground par réflexe.
