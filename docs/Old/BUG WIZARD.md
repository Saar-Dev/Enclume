...
11	Les tirages d’avantages optionnels semblent coûter des points de compétence	✅ Corrigé et testé en navigateur (2026-08-12)	Cause : Confusion UX – le tirage retire 5 points du budget « avantages pro », pas des points de compétence. Conforme RAW. Correctif : améliorer la clarté de l’interface (libellés, tooltips). **Fix réel (2026-08-12)** : note explicite ajoutée sous "Tirage 1D10 (optionnel)" (`ProAdvantagesAndSetbacks.jsx`) — précise que le tirage retire 5 points du budget Avantages professionnels affiché juste au-dessus, jamais des points de Compétence.
...
18	Absence de textes de tutoriel en haut de chaque étape	✅ Corrigé (code) — non testé navigateur	Textes rédigés, contraintes i18n définies, composant StepTutorial à créer. **Fix réel (2026-08-12)** : `StepTutorial.jsx` créé, intégré une seule fois dans `WizardCreation.jsx` (pas dans chaque composant d'étape — évite la duplication littérale des 7 fichiers listés par le doc). Steps 0,1,2,3,4,5,7 couverts (textes de Saar). Step 6 (Matériel et biens, ajouté au Wizard après la rédaction de ce bug) n'a pas de texte — composant rendu `null` sans clé, en attente. `en.json` non synchronisé (anglais gelé, décision `LOCALISATION.md` §6). Détail `docs/JOURNAL8.md`.
...
24	Boutons -/+ non harmonisés (Step 4)	✅ Corrigé et testé en navigateur (2026-08-12)	Les trois allocateurs utilisent les mêmes classes CSS (wiz4-sbtn). L’incohérence éventuelle vient du contexte CSS parent. Vérifier et uniformiser le style global. **Fix réel (2026-08-11)** : diagnostic écarté tel quel — les 3 allocateurs `wiz4-sbtn` sont bien identiques (vérifié, aucune règle CSS parent-spécifique). La vraie incohérence, plus large que "Step 4" (recoupe `BUGIDENTIFIE.md` #8, "sur tout le wizard") : 3 classes CSS pour le même pattern +/-, dont deux (`wiz1-spin-btn` Step1 Attributs, `wiz4-stepbtn` Step4 années de carrière) quasi identiques mais dérivées à des valeurs différentes (24×24/radius4 vs 26×26/radius6), plus un glyphe `+` ASCII incohérent avec le `＋` pleine chasse utilisé partout ailleurs. Fusionnées en une seule classe partagée `.wiz-spin-btn` (convention `wiz-*` déjà utilisée pour les éléments transverses du Wizard). `wiz4-sbtn` conservé distinct (grilles denses de points de compétences — un style plus discret y limite le bruit visuel), décision confirmée par Saar. Détail `docs/JOURNAL8.md`.
25	Icône /!\ trop petite (Step 4 Profession)	✅ Corrigé et testé en navigateur (2026-08-12)	Correctif : augmenter la font-size de la classe wiz4-restr dans le CSS. **Fix réel (2026-08-11)** : diagnostic confirmé et appliqué tel quel — `.wiz4-restr` héritait de `.wiz4-railmeta` (10px, taille du texte salaire/rang environnant). `font-size: 13px` + `line-height: 1` posés directement sur `.wiz4-restr` (seul usage, `CareersAllocator.jsx`), sans toucher au reste de la ligne. Détail `docs/JOURNAL8.md`.
26	Tableau des attributs trop large (Step 2)	Rejeté (Saar, 2026-08-12)	Correctif : ajouter overflowX: auto et réduire la taille de police des en-têtes. Désaccord avec l'analyse bêta-testeurs — comportement jugé correct tel quel.
27	Tooltip explicatif manquant sur le génotype (Step 2)	✅ Corrigé (code) — non testé navigateur	Couvert par les tutoriels globaux (bug #18) — bandeau `StepTutorial` sur Step2Genotype, pas de tooltip dédié ajouté séparément.
28	Avantages/désavantages à regrouper par famille (Step 5)	✅ Corrigé et testé en navigateur (2026-08-12)	Correctif : utiliser le champ family des données pour créer des groupes visuels dans Step5Advantages.jsx. Détail dans la section dédiée ci-dessous.
Fonctionnalités absentes			
29	Prérequis des carrières visibles dans l’UI	✅ Corrigé et testé en navigateur (2026-08-12)	Les données sont disponibles dans ref_career_prerequisites ; les afficher lors de la sélection d’une carrière. **Fix réel (2026-08-12)** : recherche UX faite avant de coder (voir `docs/JOURNAL8.md`) — filtre par défaut "Tous" au lieu de "Accessibles", rail grisé + icône ⚠ + tooltip pour un métier inéligible, fusionné avec l'avertissement géographique existant. `evaluateCareerEligibility`/`formatReason` déjà présents côté client, réutilisés tels quels — aucune nouvelle logique d'éligibilité.
30	Wishlist de matériel avec validation du MJ (Step 6)	✅ Corrigé (code) — non testé navigateur	Nouvelle fonctionnalité — `docs/PLANS/PLAN_WIZARD_MATERIEL_GAUGES.md`. Étend aussi la fiche permanente (jauges MJ-ajustables), pas seulement le Wizard.
31	Type de munition des armes affiché (Step 6)	✅ Corrigé (code) — non testé navigateur	Couvert comme effet de bord de #30 — `caliber` affiché dans InventoryPanel dès qu'un item en possède un (condition déjà utilisée ailleurs dans le code, `inventoryService.js`).
32	Implants avec niveaux conformes aux règles RAW	Analysé — non traité (décision Saar, 2026-08-12)	Vérifier la logique existante et ajouter la gestion des niveaux. **Analyse (2026-08-12)** : périmètre plus large que le titre — seulement 11 lignes `ref_equipment` ont `max_level` renseigné (pas que des implants), la formule de coût varie par ligne (`price_modifier` texte libre : `x niv`, `x (niv x niv)`, `x loc x (niv x niv)`...), certains objets à niveaux sont modélisés en plusieurs lignes catalogue séparées plutôt qu'un niveau paramétrable, et `max_level` ne sert aujourd'hui qu'au filtrage marchand (`tradeService.js`), jamais à l'achat. Bug de donnée trouvé au passage, distinct : "Améliorations Ours" a `price=84`/`price_modifier='000 x niv'` (coupure ratée de "84 000 x niv" à l'extraction). Décision Saar : pas traité pour l'instant.
...
**Rejeté (Saar, 2026-08-12)** — désaccord avec l'analyse des beta-testeurs, comportement jugé correct tel quel. Aucun correctif.
Analyse
...
(`advantageConstraints.js`) — variantes mutuellement exclusives d'un même avantage/désavantage
(Phobie, Allergie, Déséquilibre mental, Sens développé...). La majorité des items restent hors
