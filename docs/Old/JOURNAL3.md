...
## Session 85 — Item 17 : Drag&Drop tokens bloqué pendant le combat — 2026-06-09

**Contexte :** Pendant un combat actif, les joueurs (PJ) pouvaient glisser librement leurs tokens sur la map, contournant le système de déplacement combat (déclaration d'intention → résolution). Intégrité de jeu compromise.

**Root cause :** `Canvas3D.jsx` — `handleDragStart` ne vérifiait la phase de combat que pour le sous-mode "sélection déplacement" (`combatMoveModeRef`). Aucun guard global pendant toute la durée du combat.

**Fix — `Canvas3D.jsx` :**
1. Import `useCombatStore` ajouté.
2. `const { phase } = useCombatStore()` dans `Scene`.
3. `combatPhaseRef` créé (pattern P40 — ref miroir mis à jour à chaque render).
4. Guard dans `handlePointerMove` après `if (!dragRef.current.active) return` :
   `if (combatPhaseRef.current && !isGm) return`

**Pourquoi dans `handlePointerMove` et pas `handleDragStart` :**
Le guard dans `handleDragStart` (première tentative) bloquait aussi les clics — `handlePointerUp` vérifie `dragRef.current.active` avant d'ouvrir le MenuRadial. En plaçant le guard dans `handlePointerMove`, `hasMoved` ne devient jamais `true` pendant un combat pour les PJ → `handlePointerUp` traite l'event comme un clic → MenuRadial fonctionne normalement.

**Comportement final :**
- PJ en combat : drag token → silencieusement annulé. Clic token → MenuRadial ✓
- GM en combat : drag et clic → inchangés ✓
- Hors combat : comportement inchangé ✓

## Session 85 — Item 16 : Position par défaut CombatActionWindow — 2026-06-09

**Contexte :** La fenêtre "Phase 1 — Déclaration d'intention" (CombatActionWindow) apparaissait en haut à gauche, sous la Timeline, inaccessible pour les tests.

**Root cause :** `defaultPos.top = window.innerHeight - 760`. Sur un écran de 768px de hauteur → top = 8px → clampé au minimum (8px dans useDraggable) → fenêtre collée directement sous la Timeline (~60px de hauteur).

**Fix — `CombatActionWindow.jsx:136` :**
`top: window.innerHeight - 760` → `top: Math.max(80, window.innerHeight - 760)`

Garantit 80px minimum depuis le haut (en dessous de la Timeline) quelle que soit la hauteur d'écran.

**Note :** si localStorage contient une ancienne valeur `combat-action-pos` avec top ≤ 8, le fix ne s'applique pas (localStorage prend priorité). Corriger via `localStorage.removeItem('combat-action-pos')` dans la console navigateur.

## Session 85 — Bug #8 : CombatGmDeclareWindow reset à STATE_DEFAULTS — 2026-06-09

**Contexte :** Quand le slot GM avançait vers un nouveau PNJ en phase ANNOUNCEMENT, la fenêtre affichait toujours position=standing / weapon=holstered, ignorant l'état réel du PNJ (arme déjà dégainée, accroupi, etc.).

**Root cause — `CombatGmDeclareWindow.jsx:98` :**
Le `useEffect` sur `[activeTokenId]` appelait `setLocalStates({ ...STATE_DEFAULTS })`. Or `initialStates` (lignes 115-123) était déjà calculé correctement depuis `activePnjEntry` (roster entry du PNJ actif). Le reset ignorait cette valeur.

**Pourquoi c'est correct de lire `initialStates` dans le useEffect :**
`COMBAT_SLOT_ADVANCED` appelle `advanceSlot()` dans le store qui met à jour uniquement `activeSlotIdx` + `activeTokenId`. Le `roster` reste intact et contient déjà les états à jour de tous les PNJs (chargés via `COMBAT_PHASE_CHANGED` → `startResolutionPhase` → `SELECT *` sur `combat_roster`). La closure du callback réagit au nouveau render où `initialStates` est recalculé depuis le bon `activeTokenId`. Pas de risque TDZ : l'effet s'exécute après le render, `initialStates` est déjà initialisé.

**Fix — une ligne :**
`setLocalStates({ ...STATE_DEFAULTS })` → `setLocalStates({ ...initialStates })`

**Conformité MANUELSYSCOMBAT.md :** `combat_roster` est la SSOT des états per-entité. Le fix aligne le composant sur ce modèle.

## Session 85 — Bug #13 : isRushed -5 absent du total CDR affiché — 2026-06-09

**Contexte :** En phase RESOLUTION, la pill CDR dans CombatModifiersWindow affichait un total erroné pour un tireur "Précipité". Le warning "⚠ Précipité (−5 comp)" était visible dans le body, mais le total (`totalModComp`) n'incluait pas le -5. Le serveur appliquait correctement le malus.

**Root cause — `CombatModifiersWindow.jsx:176` :**
`totalModComp` ne sommait pas `isRushed ? -5 : 0`. Résultat : joueur voit "Tir 15 ±0" mais le jet réel sera "Tir 15 -5".

**Vérifications effectuées avant de coder :**
1. `state_vitesse === 'rushed'` = "Précipitée" (combatSections.js STATE_DEFS, coût +3 INI — §3 MANUELSYSCOMBAT.md). Complètement séparé de `TIREUR_ALLURES` (qui lisent les actions de mouvement). Zéro double comptage.
2. `activeRosterEntry` = `playerRosterEntry` = entrée roster du joueur lui-même (l'attaquant). Cohérent avec `rosterAttaquant?.state_vitesse` côté serveur.

**Fix — une ligne :**
`... + tailleModComp` → `... + tailleModComp + (isRushed ? -5 : 0)`

**Bug d'affichage pur** — le jet serveur était déjà correct.

## Session 85 — Bugs A+B : Harmonisation UX combat (ciblage + déplacement) — 2026-06-09

**Contexte :** Après le fix isMeleeSetup, tour de combat GM fonctionnel mais UX incomplète :
- Bug A (GM Assaut tir) : fenêtre de validation fixée bas-droite, loin de la cible sélectionnée
- Bug B (Déplacement) : aucune surbrillance sur la case sélectionnée + fenêtre validation bas-droite

**Éléments déjà codés — fonctionnels après fix isMeleeSetup :**
- Camera centering : `handleStartAttack/Move` → `onEnterTargetMode/MoveMode` → `setCombatCameraCenter` → Canvas3D `useEffect` → `orbitRef.current.target.set`. Opérationnel.
- Trait visuel attaquant→cible : `handleDragStart` → `onPendingTarget` → `pendingTargetId` → `targetLinePoints` `useMemo` → rendu `<line>`. Opérationnel.
- Réponse question "joueurs voient-ils les actions PNJ ?" : OUI via `announcementMarker` (`COMBAT_ACTION_DECLARED` → panneau bas-gauche "vient d'annoncer" avec moveTarget + attackTargetId). Persiste jusqu'au `COMBAT_PHASE_CHANGED`.

**Fix — 3 fichiers :**

`Canvas3D.jsx` :
1. `handleDragStart` : `onPendingTarget(token.id, e.clientX, e.clientY)` (+ coordonnées écran)
2. `handlePointerUp` : `screenX: e.clientX, screenY: e.clientY` ajoutés au payload `onPendingMove`
3. Nouveau prop outer+inner `pendingMoveSelection`
4. Rendu case sélectionnée : `<mesh>` plan `#3b82f6` opaque 0.7 à `[targetPosX+0.5, y, targetPosY+0.5]` quand `combatMoveMode && pendingMoveSelection` (PE14 : `targetPosY` = Z Three.js)

`SessionPage.jsx` :
1. `handleEnterTargetMode.onPendingTarget` : signature `(id, screenX, screenY)` → stocke `pendingTargetScreenPos: {x,y}` dans `combatTargetMode`
2. Canvas3D render : `pendingMoveSelection={pendingMoveSelection}` ajouté

`CombatOverlay.jsx` :
1. Panneau assaut : si `pendingTargetScreenPos` → `position:fixed, left:clamp(8px,x-110px,calc(100vw-228px)), top:clamp(8px,y+16px,calc(100vh-160px))`. Fallback bas-droite (`styles.moveLegend`).
2. Panneau déplacement : si `pendingMoveSelection.screenX` → même mécanique. Fallback bas-droite.

**Impact :** code path identique GM et joueur — les deux panneaux se positionnent près du clic. Harmonisation GM/PJ complète.

## Session 85 — Bug #1 (suite) : "Assaut (tir)" activait Corps à corps — 2026-06-09

**Contexte :** Un PNJ avec une arme à feu (ex. Breather Deep) — en cliquant "Assaut (tir)" dans CombatGmDeclareWindow, le panneau Corps à corps s'ouvrait au lieu du panneau tir. Le bouton semblait inactif / ne faisait rien de visible côté tir.

**Root cause — `CombatGmDeclareWindow.jsx:349` :**
`isMeleeSetup` incluait la condition `(combatTargetMode?.tokenId === activeTokenId)`. Or `handleStartAttack()` (Assaut tir) appelle `onEnterTargetMode(activeTokenId, ..., 'ranged')`, ce qui pose `combatTargetMode.tokenId = activeTokenId`. Résultat : cliquer "Assaut (tir)" → `isMeleeSetup = true` → la fenêtre s'étend à 720px et affiche le panneau CaC au lieu du panneau tir.

**Fix :**
Suppression de la ligne `(combatTargetMode?.tokenId === activeTokenId)` de `isMeleeSetup`. Les trois conditions restantes (`meleePendingMode`, `meleeTargets.length > 0`, `!!chargeSelection`) sont suffisantes pour détecter un setup CaC réel.
...
const isMeleeSetup = isActivePnj && (
  ...
  !!chargeSelection ||
  (combatTargetMode?.tokenId === activeTokenId)   // ← supprimé
)
// APRÈS
const isMeleeSetup = isActivePnj && (
  meleePendingMode ||
...
## Session 85 — Bug C : CombatActionWindow toujours en haut à gauche (joueur) — 2026-06-09

**Contexte :** La fenêtre "Phase 1 — Déclaration d'intention" côté joueur (CombatActionWindow) apparaissait systématiquement en haut à gauche, sous la Timeline. Impossible à utiliser.

**Root cause :** La classe CSS `.combat-float-win` (`client/src/index.css:1192`) ne déclare pas de propriété `position`. Les styles inline `left: pos.left; top: pos.top` n'ont d'effet que si l'élément a `position: absolute` ou `position: fixed`. Sans `position`, l'élément suit le flux normal du document → rendu en haut à gauche du conteneur.

**Pourquoi fix inline et non dans la classe CSS :**
`CombatDamageWindow` utilise également `.combat-float-win` mais est centré par un conteneur flexbox parent (overlay `inset: 0, display: flex, alignItems: center, justifyContent: center`). Ajouter `position: fixed` dans la classe CSS casserait ce centrage.

**Fix — `CombatActionWindow.jsx` :**
Ajout de `position: 'fixed'` dans le `style={{}}` inline de chacun des 7 blocs `.combat-float-win` (6 early-returns + 1 return principal).

## Session 85 — Bug D : CombatModifiersWindow toujours en haut à gauche + largeur max (GM) — 2026-06-09

**Contexte :** En phase RESOLUTION côté GM, la fenêtre modificateurs (CombatModifiersWindow) s'affichait en haut à gauche avec une largeur plein écran.

**Root cause :** Identique au Bug C — même classe `.combat-float-win`, même absence de `position`. Conséquence identique : `left/top` ignorés, fenêtre en flux normal → top-left.

**Fix — `CombatModifiersWindow.jsx:206` :**
Ajout de `position: 'fixed'` dans le `style={{}}` inline du div racine `.combat-float-win combat-float-win--gold`.

## Session 85 — Bug #8/#12 : STATE_DEFAULTS reset (CombatGmDeclareWindow) — 2026-06-09

**Contexte :** À chaque changement de slot actif (nouveau PNJ), `localStates` (états de déclaration affichés dans l'UI GM) était remis à `STATE_DEFAULTS` (debout, holstered, cc, exposed, normal) au lieu de lire l'état réel du PNJ depuis le roster. Si un PNJ était accroupi en DB, l'UI affichait "Debout" et calculait un coût d'initiative erroné (-3 pour re-s'accroupir). Une validation sans modification envoyait `position: 'standing'` au serveur — fausse transition.

**Root cause — `CombatGmDeclareWindow.jsx:98` (ancienne version) :**
```js
// AVANT
useEffect(() => {
  setLocalStates({ ...STATE_DEFAULTS })   // ← toujours les défauts, ignore le roster
  ...
}, [activeTokenId])
```
`initialStates` (correct, lit depuis `activePnjEntry` du roster) existait mais n'était pas utilisé dans le `useEffect`.

**Fix :**
```js
// APRÈS
useEffect(() => {
  setLocalStates({ ...initialStates })    // ← lit depuis roster via activePnjEntry
  ...
}, [activeTokenId])
```
Timing confirmé sûr : `COMBAT_PHASE_CHANGED` envoie le roster (→ `updateRoster`) AVANT `COMBAT_SLOT_ADVANCED` (→ `advanceSlot`/`activeTokenId`). Zustand met à jour synchroniquement. Le `useEffect([activeTokenId])` capture donc `activePnjEntry` à jour.

**Fichier modifié :** `client/src/components/CombatGmDeclareWindow.jsx`

## Session 85 — Item 16 : Fenêtre joueur masquée par la Timeline — 2026-06-09

**Contexte :** `CombatActionWindow` utilisait `top: window.innerHeight - 760` comme position par défaut. Sur des écrans de hauteur standard (~900px), cela donne `top: 140`, ce qui chevauchait la CombatTimeline positionnée à `top: 0`.
...
```
`Math.max(80, ...)` garantit un minimum de 80px depuis le haut, sous la Timeline.

## Session 85 — M3 : Propagation live des paramètres campagne (CAMPAIGN_SETTINGS_UPDATED) — 2026-06-10

**Contexte :** `action_timer_sec` (et tous les autres paramètres campagne) modifiés dans `CampaignSettingsPage` n'étaient pas propagés aux clients connectés à la session. `SessionPage.jsx` chargeait `campaign` une seule fois au `loadSession` et ne le mettait jamais à jour par WS. Le timer de combat restait bloqué à la valeur du début de session même après modification par le GM.

**Root cause :** `PUT /api/campaigns/:id` persistait en DB et retournait `{ campaign }` mais n'émettait aucun événement WS. Aucun listener côté client.

**Fix — 3 fichiers :**

`shared/events.js` :
```js
CAMPAIGN_SETTINGS_UPDATED: 'campaign:settings_updated',  // serveur → room
```
...
`client/src/pages/SessionPage.jsx` (dans le bloc WS socket handlers) :
```js
...
**Note architecture :** broadcast du `campaign` complet → tous les champs propagés (`action_timer_sec`, `shock_auto_stun`, `dice_config`, `pnj_unlimited_ammo`, `reload_mode`). Merge idempotent. `handleSetDefault` (SessionPage) fait aussi un PUT campaigns → double-update sans effet.

## Session 85 — M4 / Item 11 : Changer d'arme bloqué si arme équipée — 2026-06-10

**Contexte :** Impossible d'équiper une arme si un emplacement était déjà occupé — l'arme équipée disparaissait du dropdown (`!i.slot`), et le serveur retournait 409 si on tentait quand même (slot occupé). Les armes 2M/Tr étaient toutes assignées au slot `2M` sans possibilité de choisir `Tr`.

**Root cause — triple :**
1. `availableWeapons` filtrait `!i.slot` → les armes déjà équipées disparaissaient du dropdown. Quand toutes les armes de l'inventaire étaient équipées, la section entière disparaissait.
2. `handleEquip` — aucune résolution de conflits. PUT direct sur un slot occupé → 409 serveur.
3. Picker slot : toutes les armes `2M` ou `Tr` assignées à `2M`. Armes `2M/Tr` n'avaient pas de choix entre les deux emplacements.

**Fix — `client/src/character/WeaponPanel.jsx` :**

1. **`getSlotInfo(refLocation)`** (helper pur ajouté avant `export default`) : parse `ref_location` (séparateur `/`) → `{ type: '1H'|'2M'|'Tr'|'2M_Tr'|'unknown', defaultSlot }`.

2. **`availableWeapons`** : retrait de `!i.slot`. Toutes les armes hors Coffre sont maintenant dans le dropdown, équipées ou non. Le label affiche `(Main G)` etc. pour les armes déjà équipées.

3. **`handleSelectWeapon`** : `getSlotInfo(item.ref_location).defaultSlot` au lieu de l'ancienne logique `isTwoHand ? '2M' : 'MG'`.

4. **`handleEquip`** (réécriture) : calcul des conflits avant toute requête, boucle `for await` de déséquipement (`PUT slot: null`) sur tous les conflits, puis équipement final. Optimistic update `setItems` après chaque PUT. `onInventoryMutated()` appelé une seule fois à la fin (refetch API stable post-serveur).
   - Arme 2H (2M ou Tr) → conflictSlots = `['MG','MD','2M','Tr']`
   - Arme 1H → conflictSlots = `[equipSlot, '2M', 'Tr']`
   - Auto-exclusion de l'arme elle-même : `w.id !== equipItemId`

5. **Slot picker** selon `type` :
   - `1H` → select MG / MD
   - `2M` → pas de picker (slot auto `2M`)
   - `Tr` → pas de picker (slot auto `Tr`)
   - `2M_Tr` → select "2 mains" / "Trépied"

6. **Warning Tr** : `⚠ Trépied requis` + tooltip HTML, visible sur la carte d'arme équipée en `Tr` ET dans la zone d'équipement si `equipSlot === 'Tr'`.

**Cohérence serveur :** la logique de conflits client mirror exactement le code serveur (`campaigns.js:1149` — isTwoHand conflict les 4 slots, sinon slot exact + 2M + Tr). Aucune 409 possible si le client calcule correctement.

## Session 85 — Fix affichage munitions "non chargée — X/Y chargeur" — 2026-06-10

**Contexte :** WeaponPanel affichait "non chargée — 30/30 chargeur · 0 en stock" pour une arme jamais chargée. Les deux infos étaient mutuellement exclusives mais le code les rendait indépendamment.

**Root cause — `WeaponPanel.jsx` (ligne ammoCount) :**
`{ammoCount > 0 && ...}` évalué sans tenir compte de `weapon.current_ammo`. Quand `current_ammo = null` ET `ref_ammo_count = "30 coups"`, les deux blocs s'affichaient ensemble.

**Fix — une condition :**
`{ammoCount > 0 &&` → `{weapon.current_ammo && ammoCount > 0 &&`

Résultat :
- Non chargée → "non chargée" uniquement
- Chargée + chargeur défini → nom munition + X/Y chargeur · N en stock

## Session 85 — M5 : Inventaire joueur non rafraîchi après création GM — 2026-06-10

**Contexte :** Quand le GM ajoutait un item à l'inventaire d'un PJ via InventoryPanel, l'inventaire du joueur (CharacterWindow ouverte) ne se mettait pas à jour.

**Root cause — double lacune :**

1. `SessionPage.jsx` : aucun handler `s.on(WS.INVENTORY_ADDED, ...)`. Les handlers `INVENTORY_UPDATED` et `INVENTORY_REMOVED` existaient et bumaient `woundVersions` (déclenchant un reload), mais `INVENTORY_ADDED` était absent.

2. `CharacterWindow.jsx` + `InventoryPanel.jsx` : même si SessionPage avait bumé `woundVersions` → `inventoryVersion`, `InventoryPanel` ne recevait pas de prop `reloadKey` et son `useEffect` n'avait que `[characterId]` en deps → jamais de refetch depuis l'extérieur.

**Fixes — 3 fichiers, 4 modifications :**
- `SessionPage.jsx` : handler `INVENTORY_ADDED` ajouté (pattern identique `INVENTORY_UPDATED`)
- `CharacterWindow.jsx` : `reloadKey={inventoryVersion}` passé à `InventoryPanel`
- `InventoryPanel.jsx` : prop `reloadKey = 0` ajoutée + `[characterId, reloadKey]` en deps useEffect

## Session 85 — Broadcast soins blessures + UX stabilisation + silhouette — 2026-06-10

### Bug : soin blessures non broadcasté

**Contexte :** La création d'une blessure (`WOUND_ADDED`) mettait bien à jour `ArmorWoundPanel` sur les autres clients. En revanche, la stabilisation (`WOUND_UPDATED`) et la guérison (`WOUND_REMOVED`) ne déclenchaient aucun rechargement distant.

**Root cause — `SessionPage.jsx` :**
Les handlers `WOUND_UPDATED` et `WOUND_REMOVED` appelaient `updateCharacter` (mise à jour `worst_wound_severity`) mais ne bumaient pas `woundVersions`. Sans ce bump, `reloadKey` de `ArmorWoundPanel` restait stable → pas de refetch wounds sur les autres clients.

Le payload serveur inclut bien `worst_wound_severity` dans les deux événements (confirmé dans `char-sheet.js` lignes 707 et 731) — BLESSURES.md ne le documentait pas.

**Fix — 2 lignes :**
`WOUND_UPDATED` et `WOUND_REMOVED` : ajout `setWoundVersions` bump (même pattern que `WOUND_ADDED`).

### UX : badge stabilisation peu visible

**Contexte :** Blessure stabilisée : fond couleur blessure + `✓` vert 8px + glow `0 0 4px #00ff00`. Contraste insuffisant, notamment sur fond orange/rouge.

**Fix — `LocationPanel.jsx` :**
- `outline: '2px solid #4caf77'` (pas d'impact box model)
- `boxShadow: '0 0 5px rgba(76,175,119,0.6)'` (plus visible)
- `border: '1px solid #5a5a7a'` (neutre, laisse l'outline s'exprimer)
- ✓ : `color: '#fff'`, `fontWeight: 'bold'`, `fontSize: 9`

### UX : silhouette trop grande sur fenêtre large

**Contexte :** `SilhouettePanel` SVG à `width: '50%'` sans plafond. Sur CharacterWindow large, la colonne centre (`1fr`) s'élargissait → silhouette pouvait dépasser 200px de large.

**Fix — `SilhouettePanel.jsx` :**
`maxWidth: '160px'` sur le SVG (viewBox 300×460 → max hauteur ≈ 245px).
