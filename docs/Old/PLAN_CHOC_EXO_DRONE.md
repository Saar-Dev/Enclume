...
| B — Tir exo/drone (+ AOE tireur exo) | **Codé, committé et VALIDÉ en session réelle par Saar (2026-09-05)** — Tir/CaC normal (PNJ+PJ) confirmé. Sous-cas AOE (lance-flammes exo en zone) : le blocage séparé (mode « Viser une zone » absent quand l'exo est pilotée en MJ, `onEnterAoeTargetMode` manquant sur ce rendu — trouvé par instrumentation temporaire, pas par hypothèse) **corrigé et committé** (`9a4d4b3`, hors périmètre de ce plan). **Retesté ensuite : VALIDÉ** — 3 cibles touchées, `burning` appliqué, **2 Tests de Choc déclenchés et visibles au chat** (confirme que `shooterChocDsl` circule bien jusqu'à `resolveTargetHit` pour ce sous-cas). Bug annexe trouvé pendant cette validation : le Test de Choc s'affiche sous le nom du **tireur** au lieu de la **cible** — vérifié pré-existant (7 sites, tout le système de combat, pas spécifique exo/drone/AOE) — ticketé séparément (`CHOC-TEST-WRONG-ATTRIBUTION`), pas corrigé ici. |
...
| Tests + session réelle | **Tous les paliers (0/A/B y compris AOE/D) validés en session réelle par Saar. Chantier fonctionnellement clos.** 3 tickets annexes ouverts en cours de route, tous hors périmètre de ce plan : `EXODRONE-CONFIRMDAMAGE-CRASH` (résolu), `NATWEAPON-CHOC-DEFENSE-GAP` (triaged), `CHOC-TEST-WRONG-ATTRIBUTION` (triaged) |
