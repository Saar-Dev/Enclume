...
export default function WeaponPanel({ characterId, canEdit, onOpenModing = () => {}, dragItem = null, inventoryBanner = null }) {
  ...
  console.log('[DBG-ARMEDND] ' + JSON.stringify({ canEdit, hasAnything, weaponDirId: weaponDir?.id ?? null, weaponSecId: weaponSec?.id ?? null, weapon2MId: weapon2M?.id ?? null, equippedWeaponIds: equippedWeapons.map(i => i.id), availableWeaponIds: availableWeapons.map(i => i.id), items: items.map(i => ({ id: i.id, name: i.custom_name || i.ref_name, ref_family: i.ref_family, ref_category: i.ref_category, ref_location: i.ref_location, container: i.container, slots: i.slots })) }))
