...
export default function Object3DPreview({ blueprint, materialOverrides, compact = false }) {
  ...
  const url = /^https?:\/\//i.test(blueprint.glb_url)
    ? blueprint.glb_url
    : `${import.meta.env.VITE_API_URL}/api/assets/${blueprint.glb_url}`

  return (
    <div style={{ position: compact ? 'relative' : 'sticky', top: compact ? undefined : 0, zIndex: 12, margin: compact ? '4px 0 8px' : '8px 0 10px', border: '1px solid #3a4c70', borderRadius: '6px', overflow: 'hidden', background: '#090c12', boxShadow: compact ? 'inset 0 0 18px rgba(0,0,0,0.45)' : '0 8px 20px rgba(0,0,0,0.55)' }}>
      <div style={{ height: compact ? '112px' : '190px', position: 'relative' }}>
        <Canvas
          frameloop="demand"
          dpr={1}
      ...
      <div style={{ padding: compact ? '5px 7px' : '7px 9px', borderTop: '1px solid #202036' }}>
