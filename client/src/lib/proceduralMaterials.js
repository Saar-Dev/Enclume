...
export const DEFAULT_SURFACE_MATERIAL_PRESET = {
  ...
  pattern: DEFAULT_PROCEDURAL_MATERIAL.pattern,
  wear: 28,
  dirt: 18,
  relief: 78,
  realRelief: true,
...
function applyMetalPanels(ctx, height, size, relief) {
  const seam = Math.max(2, Math.round(size * 0.028))
  strokeInsetRect(ctx, size, seam, 'rgba(4, 6, 8, 0.78)')
  lowerRect(height, size, 0, 0, size, seam, -0.28 * relief)
  lowerRect(height, size, 0, size - seam, size, seam, -0.28 * relief)
  lowerRect(height, size, 0, 0, seam, size, -0.28 * relief)
  lowerRect(height, size, size - seam, 0, seam, size, -0.28 * relief)

  const centerLine = Math.max(1, Math.round(seam * 0.55))
  ctx.save()
  ctx.strokeStyle = 'rgba(6, 8, 10, 0.42)'
  ctx.lineWidth = centerLine
  ctx.beginPath()
  ctx.moveTo(size / 2, seam)
  ctx.lineTo(size / 2, size - seam)
  ctx.moveTo(seam, size / 2)
  ctx.lineTo(size - seam, size / 2)
  ctx.stroke()
  ctx.restore()
  lowerRect(height, size, size / 2 - centerLine / 2, seam, centerLine, size - seam * 2, -0.12 * relief)
  lowerRect(height, size, seam, size / 2 - centerLine / 2, size - seam * 2, centerLine, -0.12 * relief)
}
...
function patternAccumulationMask(pattern, x, y, size, seed) {
  ...
  if (pattern === 'metal_panels') {
    const seam = Math.max(2, size * 0.028)
    const margin = Math.max(12, size * 0.13)
    const centerLine = Math.max(1, seam * 0.55)
    const edgeDistance = Math.min(x, y, size - x, size - y)
    mask = Math.max(mask, lineFalloff(edgeDistance, seam * 2.4) * 0.85)
    mask = Math.max(mask, lineFalloff(Math.abs(x - size / 2), centerLine * 2.2) * 0.45)
    mask = Math.max(mask, lineFalloff(Math.abs(y - size / 2), centerLine * 2.2) * 0.45)
...
function sampleMetalPanelHeight(x, y, size, relief) {
  ...
  const edgeDistance = Math.min(x, y, size - x, size - y)
  height -= 0.28 * relief * lineFalloff(edgeDistance, seam)
  height -= 0.12 * relief * lineFalloff(Math.abs(x - size / 2), centerLine / 2)
  height -= 0.12 * relief * lineFalloff(Math.abs(y - size / 2), centerLine / 2)
